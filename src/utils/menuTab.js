...
import { getCurrentRole, getSession, setSession } from 'utils/utils';
...
// 工作台 tab  9.21 将默认tab工作台移除
const workplaceTab = {
  title: '',
  // todo 可不可以使用编码, openTab 时 使用 title: intl.get(title).d(title),
  // icon: 'home',
  closable: false,
  key: '/workplace',
  path: '/workplace',
...
function getActiveTabMenuId(activeTabKey) {
  ...
  forEach(routerData, (route) => {
    ...
    if (reg.test(activeTabKey) && route.id) {
      activeTabMenu = route;
      return false;
    }
  });
