...
import { Icon, Spin } from 'hzero-ui';
...
export default function VirtualTable(props) {
    ...
    if (index === 0 && isTree) {
      ...
      if (column.render) {
        tempColumn.render = (columnData) => {
          ...
          return (
            <span style={{ paddingLeft: (record.INDENT_INDEX || 0) * 12 + 8, fontSize: '14px' }}>
  ...
  const handleExpand = (flag, record, rowIndex) => {
    ...
    if (isUndefined(expandedRowKeys) && record[childrenColumnName]) {
      if (flag) {
          ...
          slice(trueDataSource, 0, rowIndex + 1)
            .concat(
              record[childrenColumnName].map((item) => ({
                ...item,
                INDENT_INDEX: (record.INDENT_INDEX || 0) + 1,
              }))
            )
        ...
        setExpandKeys(Array.from(new Set(remove(expandKeys, (item) => !rowKeys.includes(item)))));
        setTrueDataSource(
  ...
  return (
    <PerformanceTable
      ...
      renderLoading={() => {
        return (
          <Spin style={{ position: 'absolute', width: '100%', top: '50%', marginTop: '-20px' }} />
        );
      }}
    />
  );
