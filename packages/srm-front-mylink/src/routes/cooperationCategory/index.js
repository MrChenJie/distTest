...
@fastCodeLoader(['HKTB.ACTIVITY_PROSTATUS', 'HKSM.PRODCUT_SERVICE', 'HKSM.APPLICATION.STATUS', 'LINK_UPDATE_TYPE'])
@connect(({ loading, cooperationCategoryModal }) => ({
  cooperationCategoryModal,
  queryLoading: loading.effects['cooperationCategoryModal/queryList'],
  dataSource: cooperationCategoryModal?.dataSource ,
  pagination: cooperationCategoryModal?.pagination,
}))
class cooperationCategory extends Component {
  constructor(props) {
    super(props);
  ...
  handleSearch = (page = {}) => {
    ...
    dispatch({
      type: 'cooperationCategoryModal/queryList',
      payload: {
        page,
        ...this.getQueryParams(),
      },
  ...
  handleAddLines = (e) => {
    const templateCode = 'BPM_SCM_SMHBGXHZLX'; // 致远templateCode
    const url = `${
      ...
      `${process.env.CMHK_LOGIN}/oauth/public/cmi/sso/login?redirect_uri=/mylink/cooperation-category/Detail?refCaseId=${e}`
    )}`;
    window.open(url, '_blank');
    // window.open(`/mylink/cooperation-category/Detail?refCaseId=${e}`, '_blank')
  }
    ...
    if (selectedRowKeys.length > 0) {
      if (isDraft) {
        CusModal.CusDeleteConfirm(() => {
          dispatch({
            type: 'cooperationCategoryModal/deleteLine',
