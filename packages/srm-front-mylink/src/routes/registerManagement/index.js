...
@fastCodeLoader(['HKTB.ACTIVITY_PROSTATUS', 'DICT.JUDGE_APPLY_STATUS', 'HKSM.APPLICATION.STATUS'])
...
class judgesManagement extends Component {
  ...
  getQueryParams = () => {
    ...
    if (fieldsValue?.applyDateRange) {
      ...
      fieldsValue.applyDateEnd = dayjs(fieldsValue.applyDateRange[1].hour(0).minute(0).second(-1)).format(DEFAULT_DATETIME_FORMAT);
    }
    return {
      ...fieldsValue,
    };
  ...
  handleDelete = () => {
    const { dispatch, registerManagementModel, dataSource } = this.props;
    ...
    if (selectedRowKeys.length > 0) {
      CusModal.CusDeleteConfirm(() => {
        ...
        // });
          // 后台删除
          const judges = selectedRowKeys?.map((i) => {
            return {
              judgeId: i,
            };
          });
  ...
  handleAdd = () => {
    ...
    }/main/child-frame/app-approval/quick/process?templateCode=BPM_SCM_ZJKWH&pcThirdContentPageUrl=${encodeURIComponent(
      `${process.env.CMHK_LOGIN}/oauth/public/cmi/sso/login?redirect_uri=/pub/mylink/register-management/Detail`
    )}`;
                ...
                buttons={
                  <>
                    <CusButton
                      ...
                      disabled={!((selectedRows?.filter(item => ['Approved', 'Inapproval'].includes(item.applyStatus)).length === 0) && selectedRowKeys?.length !== 0)}
                    >
                    ...
                    <CusButton
                      ...
                      disabled={!((selectedRows?.filter(item => ['Draft', 'Inapproval'].includes(item.applyStatus)).length === 0) && selectedRowKeys?.length !== 0)}
                    >
