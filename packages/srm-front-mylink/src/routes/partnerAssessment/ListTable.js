...
export default class ListTable extends React.Component {
  ...
  handleApplicationLink = (record) => {
    ...
    if(record.caseId) {
      ...
      const url = `/pub/mylink/partner-assessment/Detail?formRecordId=${record.evalId}`;
      window.open(url, '_blank')
    }
  };
  ...
  render() {
    ...
    const columns = [
      {
        title: intl.get(`${prompt}.filed.review.no`).d('评估单号'),
        width: 180,
        dataIndex: 'evalNo',
        key: 'evalNo',
        render: (_, record) => {
      ...
      {
        title: intl.get(`${prompt}.field.evaluate.status`).d('评估状态'),
        width: 160,
        dataIndex: 'evalStatusMeaning',
        key: 'evalStatusMeaning',
      },
      {
