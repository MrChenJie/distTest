...
import { isArray, isEmpty, isFunction, chunk } from 'lodash';
...
import { DEFAULT_DATETIME_FORMAT, DEFAULT_DATE_FORMAT, EDIT_FORM_ROW_LAYOUT } from 'utils/constants';
...
class LovModal extends React.Component {
  ...
  render() {
    ...
    // 查询条件表单
    const span = queryFields.length <= 1 || width <= 400 ? 24 : 8;
    const queryFieldsLimit = 3;
    ...
    queryInput = chunk(queryInput, queryFieldsLimit);
      ...
      <Form>
        {queryFields.length > 0 ? (
          <div>
            {queryInput.map((item, index) => {
              return (
                <Row
                  ...
                  style={{ display: index < 2 ? 'block' : expandFlag ? 'block' : 'none' }}
                >
                  {item}
                </Row>
              );
            ...
            {/* <Row>{queryInput}</Row> */}
            <Row className="lov-modal-btn-container">
              <Col span={24} className="custmize-querybar-buttons">
                ...
                {queryInput.length > 2 && (
                  <Button onClick={this.toggleForm} style={{ marginLeft: 8 }}>
                    {expandFlag
