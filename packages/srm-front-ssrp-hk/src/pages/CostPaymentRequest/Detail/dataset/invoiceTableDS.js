...
import lineAttachment from './lineAttachmentDS';
...
const invoiceInfo = (detailInfoDS, prompt) => ({
  ...
  paging: false,
  children: {
    attachmentLineList: new DataSet(lineAttachment({ prompt, require: true })),
    otherAttachmentLineList: new DataSet(lineAttachment({ prompt, require: false })),
  },
  fields: [
    {
      name: 'costInvoiceId',
    ...
    },
    {
      ...
      max: new Date(),
      help: intl
        .get(`${prompt}.view.detail.line.invoiceDate.help`)
        .d('请输入发票的开具日期'),
    },
    {
      name: 'termsDate',
      type: FieldType.date,
      label: intl.get(`${prompt}.view.detail.invoice.termsDate`).d('发票到期日'),
      required: true,
      dynamicProps: {
        min: ({ record }) => {
    ...
    },
    {
      name: 'writeOffAmount',
      type: FieldType.number,
      label: intl.get(`${prompt}.view.detail.invoice.writeOffAmount`).d('核销原币金额'),
    },
    {
      name: 'paymentAmount',
      type: FieldType.number,
      label: intl.get(`${prompt}.view.detail.invoice.paymentAmount`).d('本次净支付原币金额'),
    },
    {
      name: 'instalmentFlag',
      ...
      dynamicProps: {
        disabled: () => {
          ...
          if (['DRAFT', 'PENDING_REVIEW'].includes(requestStatus)) {
            return false;
          } else {
            return true;
          }
    ...
    },
    {
      ...
      dynamicProps: {
        disabled: ({ record }) => {
          // 草稿状态可修改
          const requestStatus = detailInfoDS.current.getField('requestStatus').getValue();
          if (record.get('instalmentFlag') === '1' && ['DRAFT', 'PENDING_REVIEW'].includes(requestStatus)) {
            // 分期
            return false;
          } else {
            // 不分期
    ...
    },
    {
      ...
      label: intl.get(`${prompt}.view.detail.invoice.invoiceAttachment`).d('发票附件'),
    },
    {
      name: 'otherAttachment',
      label: intl.get(`${prompt}.view.detail.invoice.otherAttachment`).d('其他附件'),
    },
  ],
  events: {
    create: ({ record }) => {
    ...
    update: ({ dataSet, record, value, name }) => {
      if (name === 'instalmentFlag' && value === '1' && record.get('invoiceNum')) {
      ...
      if (name === 'instalmentFlag' && value === '0') {
