...
export default class ListTable extends React.Component {
  ...
  handleDetail = (record = {}) => {
    ...
    window.open(`${isPub ? '/pub' : ''}/spcm/cost/payment/request/detail/${record.costRequestId}`)
  };
    ...
    const columns = [
      {
        ...
        title: intl.get(`${commonPrompt}.view.query.requestNum`).d('申请编号'),
        render: (val, record) => {
          return <a onClick={() => this.handleDetail(record)}>{val}</a>;
        },
      },
      {
        width: 100,
      ...
      },
      {
        ...
        title: intl.get(`${commonPrompt}.view.query.requestTitle`).d('申请标题'),
      },
      {
        width: 300,
        dataIndex: 'companyOrgName',
        title: intl.get(`${commonPrompt}.view.query.companyOrgName`).d('公司主体'),
      },
      {
        width: 150,
      ...
      },
      {
        ...
        title: intl.get(`${commonPrompt}.view.query.invoiceDate`).d('发票日期'),
        render: dateRender,
      },
      {
        width: 200,
      ...
      {
        width: 150,
        dataIndex: 'importEbsDate',
        title: intl.get(`${commonPrompt}.view.query.importEbsDate`).d('导入EBS日期'),
        render: dateRender,
      },
      {
        width: 100,
        dataIndex: 'currencyCode',
        title: intl.get(`${commonPrompt}.view.query.currencyCode`).d('发票币种'),
      },
      {
        width: 150,
        dataIndex: 'invoiceAmount',
        title: intl.get(`${commonPrompt}.view.query.invoiceAmount`).d('发票金额'),
        render: (val) => {
