...
export default class EditorOnline extends Component {
  ...
  writeFrameDocument(content) {
    const { pcHeaderId = '', pcTemplateId = '', fullScreenFlag = false } = this.props;
    const idSuffix = `H${pcHeaderId}T${pcTemplateId}${fullScreenFlag ? 'full' : ''}`;
  ...
  fetchEditorOnlineHTML() {
    const { dispatch, pcHeaderId = '', permissionCode } = this.props;
    dispatch({
  ...
  fetchEditorOnlineTemplateHTML() {
    const { dispatch, pcTemplateId = '', permissionCode } = this.props;
    dispatch({
