...
export default class purchaseInquiryQueryErp extends React.Component {
  // 创建新单据表单
  createForm;
  ...
  handleDetele(data = [], callback = (e) => e) {
    ...
    const { dispatch } = this.props;
    const deleteFlag = data.every(item => item.prApplyStatus === '起草');
    if (deleteFlag) {
      CusModal.confirm({
      ...
      CusNotification.error({
        message: intl.get(`${promptCode}.tips.onlyDeleteNEW`).d('只能删除状态为“起草”的采购单'),
      });
    }
  }
  ...
  handleAddTemplate() {
    ...
    dispatch(
      routerRedux.push({
        pathname: `/ssrp-hk/purchaseInquirySheet/add`,
      })
    );
  }
  ...
  render() {
    const supplierDataExportProps = {
      ...
      method: 'POST',
      downloadType: 'Blob',
      buttonText: intl.get(`${prompt}.view.button.list.export`).d('导出'),
      ...
      <PageWrapper loading={fetchLoading}>
        ...
        >
          <Panel
            ...
            header={
              <PanelHeader
                title={intl.get(`hzero.common`).d('查询')}
          ...
          <Panel
            ...
            header={
              <PanelHeader
                title={intl.get(`hzero.common`).d('结果展示')}
                arrowActive={activeKey.includes('table')}
                showArrow={false}
                buttons={
                  <>
                    <CusExcelExport otherButtonProps={{ mini: true }} {...supplierDataExportProps} />
                    <CusButton onClick={this.handleAddTemplate} mini type='primary'>新增</CusButton>
                  </>
                }
              />
            }
