...
import { handleUpdateDs, tableDetailDs, tableDs } from '../../stores/syncOuterSystemDS';
import Drawer from './Drawer';

@formatterCollections({ code: ['hpfm.syncOuterSystem'] })
@WithCustomize({
  unitCode: ['HPFM.SYNC_OUTER_SYSTEM.ORG_INFO.LINE'],
})
          ...
          actions.push(
            ...
            },
            {
              ele: (
                ...
                >
                  {intl.get('hpfm.syncOuterSystem.view.button.syncManual').d('手动同步')}
                </ButtonPermission>
              ),
              key: 'syncManual',
              len: 4,
              title: intl.get('hpfm.syncOuterSystem.view.button.syncManual').d('手动同步'),
            }
          );
  ...
  render() {
    ...
    return (
      <>
        <Header
          title={intl
            .get('hpfm.syncOuterSystem.view.message.title.syncOuterSystem')
            .d('组织信息同步')}
        >
          <ButtonPermission
            type="c7n-pro"
