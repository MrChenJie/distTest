...
export default class DataHierarchies extends React.Component {
  ...
  showModal(record = {}, editFlag, addFlag) {
    const { dispatch } = this.props;
    const { dataHierarchyId, dataHierarchyName, tenantId } = record;
  ...
  render() {
    const {
      ...
      match,
      form,
      dataHierarchies: { renderTree = [], dataHierarchiesDetail = {}, expandedRowKeys },
    } = this.props;
            ...
            <Form.Item style={{ marginBottom: 0 }}>
              {form.getFieldDecorator('tenantId', {
                ...
                <Lov
                  code="HPFM.TENANT"
                  textValue={intl
                    .get('hpfm.dataHierarchies.model.dataHierarchies.hzeroPlat')
                    .d('HZERO平台')}
                  onOk={this.handleOk}
                  allowClear={false}
                />
