  ...
  return (
    <Form>
      <Row type="flex" gutter={24} align="bottom">
        <Col span={6}>
          {isSiteFlag && (
            <Form.Item label={intl.get('hzero.common.model.tenantName').d('租户')}>
              {getFieldDecorator('tenantId')(<Lov code="HPFM.TENANT" />)}
            </Form.Item>
          )}
        </Col>
        <Col span={6}>
          <Form.Item label={intl.get('hpfm.customize.model.customize.rule.ruleCode').d('规则编码')}>
            {getFieldDecorator('ruleCode')(<Input typeCase="upper" />)}
          </Form.Item>
        </Col>
        <Col span={6}>
          <Form.Item label={intl.get('hpfm.customize.model.customize.rule.ruleName').d('规则名称')}>
