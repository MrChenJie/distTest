...
export default class Department extends React.Component {
    ...
    Modal.open({
      ...
      onOk: async () => {
        if (await this.createFormDS.validate()) {
          return this.createFormDS.submit();
        } else {
          return false;
        }
      },
    ...
    switch (type) {
      ...
      case 'edit':
        this.openModal(
          ...
          intl.get('hsrh.organization.view.message.editDepartment').d('编辑部门'),
          type
        );
        break;
      default:
  ...
  async handleEmployeeCreate({ type }) {
    if ((await this.employeeFormDS.validate()) && (await this.employeePositionFormDS)) {
      ...
      let res = true;
      if (type === 'create') {
        res = await addEmployeeDetail(resultData);
      } else {
        res = await updateEmployeeDetail(resultData);
      }
      if (res && res.failed) {
        notification.error({
          message: res.message,
  ...
  handleOpenEmployeeModal({ type, employeeId = '' }) {
    Modal.open({
      title: intl.get('hpfm.organization.view.title.addNewEmployee').d('新建员工'),
      drawer: true,
      width: 520,
      children: (
      ...
      <>
        <Row gutter={45}>
          ...
          {detailFlag ? (
                  ...
                  <div style={{ marginBottom: 100 }}>
                    <Col span={8}>
                      <div style={{ color: 'rgb(152, 152, 152)' }}>
                        {intl
                          .get('hpfm.organization.model.department.departmentCode')
                          .d('部门代码')}
                      </div>
                    ...
                    <Col span={8}>
                      <div style={{ color: 'rgb(152, 152, 152)' }}>
                        {intl
                          .get('hpfm.organization.model.department.departmentName')
                          .d('部门名称')}
                      </div>
                    ...
                    <Col span={8}>
                      <div style={{ color: 'rgb(152, 152, 152)' }}>
                        {intl.get('hpfm.organization.model.department.status').d('状态')}
                      </div>
                    ...
                    <Col span={8}>
                      <div style={{ marginTop: 16, color: 'rgb(152, 152, 152)' }}>
                        {intl
                          .get('hpfm.organization.model.department.parentUnitName')
                          .d('上级组织')}
                      </div>
                      <div style={{ overflowWrap: 'break-word' }}>
                        {basicInformation.parentUnitName}
                      </div>
                    </Col>
                    <Col span={8}>
                      <div style={{ marginTop: 16, color: 'rgb(152, 152, 152)' }}>
                        {intl.get('hpfm.organization.model.department.unitCompanyName').d('公司')}
                      </div>
                      <div style={{ overflowWrap: 'break-word' }}>
            ...
            <Col span={16}>
              <Detail employeeId={employeeId} handleBack={this.handleBack} />
            </Col>
          )}
        </Row>
      </>
