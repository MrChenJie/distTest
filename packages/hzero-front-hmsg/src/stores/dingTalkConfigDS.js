...
const tableDs = {
  ...
  queryFields: [
    !isTenantRoleLevel() && {
      name: 'organizationId',
      type: 'object',
      label: intl.get('hmsg.dingTalkConfig.model.dingTalkConfig.tenantName').d('租户名称'),
      lovCode: 'HPFM.TENANT',
      noCache: true,
      // ignore: 'always',
    },
    ...
    // },
    {
      name: 'serverCode',
      type: 'string',
      label: intl.get('hmsg.dingTalkConfig.model.dingTalkConfig.serverCode').d('配置编码'),
    },
    {
      name: 'serverName',
      type: 'string',
      label: intl.get('hmsg.dingTalkConfig.model.dingTalkConfig.serverName').d('配置名称'),
    },
    {
      name: 'authType',
      type: 'string',
  ...
  fields: [
    ...
    },
    {
      ...
      type: 'string',
      label: intl.get('hmsg.dingTalkConfig.model.dingTalkConfig.authTypeMeaning').d('授权类型'),
    },
    {
      name: 'appKey',
      type: 'string',
    ...
    isTenantRoleLevel() &&
      !VERSION_IS_OP && {
        name: 'tenantId',
        type: 'string',
        label: intl.get('hmsg.dingTalkConfig.model.dingTalkConfig.source').d('来源'),
      },
    {
      name: 'enabledFlag',
      type: 'boolean',
      label: intl.get('hmsg.dingTalkConfig.model.dingTalkConfig.enabledFlag').d('启用标识'),
      trueValue: 1,
      falseValue: 0,
      defaultValue: 1,
    },
...
const detailDs = {
  ...
  fields: [
    !isTenantRoleLevel() && {
      name: 'tenantIdLov',
      type: 'object',
      label: intl.get('hmsg.wechatOfficials.model.wechatOfficials.tenantName').d('租户名称'),
      lovCode: 'HPFM.TENANT',
      required: true,
