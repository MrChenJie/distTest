...
@formatterCollections({ code: 'hmsg.wechatConfig' })
export default class WechatConfig extends React.Component {
  ...
  handleEdit(isEdit, record, isCopy) {
    ...
    const title = !isEdit
      ? intl.get('hmsg.wechatConfig.view.message.title.crete').d('新建')
      : intl.get('hmsg.wechatConfig.view.message.title.edit').d('编辑');
    Modal.open({
      closable: true,
      key: 'customer-group',
