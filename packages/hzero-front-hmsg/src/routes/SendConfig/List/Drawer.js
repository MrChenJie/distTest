      ...
      form.validateFields((err, values) => {
        if (isEmpty(err)) {
          ...
          receiverAddressList = newReceiverAddressList.map(item => ({
            ...
            phone: item.SMS || item.CALL ? item.SMS || item.CALL : undefined,
        ...
        <Row>
          ...
          <Col span={10} offset={1}>
            ...
            >
              {getFieldDecorator(`recipientValue${k}`, {
                rules: [
                  {
                    type: getFieldValue(`recipientKey${k}`) === 'WEB' ? 'number' : 'string',
                    required:
          ...
          {form.getFieldValue(`recipientKey${k}`) === 'DT' && (
            <Col span={10}>
              <FormItem
                required={false}
                label={intl.get('hmsg.sendConfig.model.sendConfig.gentId').d('应用ID')}
                {...formItemLayout}
              >
