...
import { isArray, isEmpty, isFunction, pullAllBy } from 'lodash';
...
import { numberToThousands } from '@/common/utils';
...
class LovModal extends React.Component {
  ...
  onRowSelect(record, selected) {
    ...
    const newSRows = selected
      ...
      : selectedRows.filter((n) => n[rowkey] !== record[rowkey]);
    this.props.onSelect(newSRows);
    this.setState({
  ...
  onSelectAll(selected, newSelectedRows, changeRows) {
    ...
    const newSRows = selected
      ...
      : pullAllBy([...selectedRows], changeRows, rowkey);
    this.props.onSelect(newSRows);
    this.setState({
      selectedRows: newSRows,
    });
  ...
  render() {
    ...
    const rowSelection = {
      ...
      onSelect: isSelect ? this.onRowSelect : undefined,
      onSelectAll: isSelect ? this.onSelectAll : undefined,
    };
    const tableProps = {
      loading,
      rowSelection,
      ...
      onRow: isSelect ? this.handleRow : undefined,
    };
