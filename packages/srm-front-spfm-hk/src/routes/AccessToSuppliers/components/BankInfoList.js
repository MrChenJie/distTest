...
export default class BankInfoList extends PureComponent {
  ...
  setDefaultVal = (e, record) => {
    const { accessToSupplierHK, dispatch } = this.props;
  ...
  handleLeaveCode = (record, lovData) => {
    ...
    dispatch({
      type: 'accessToSupplierHK/getLeaveCode',
      payload: {
        ruleCode: 'BANK_CODE',
      },
    }).then((res) => {
  ...
  render() {
    const { bankInfoColumnsRowSelection, accessToSupplierHK, disabled = false } = this.props;
    const {
      bankInfoListDataSource = [],
    } = accessToSupplierHK;
    ...
    const columns = [
      {
        ...
        required: true,
        render: (_, record) => {
          if(disabled || record.uploadErpStatus === 'Y') {
