    ...
    const formItemList = [
      ...
      <Col span={colSpan}>
        <Form.Item label={intl.get(`${prompt}.field.request.status`).d('申请状态')}>
          {getFieldDecorator('applyStatus')(<CusSelect style={{ width: '100%' }}
      ...
      <Col span={colSpan}>
        <Form.Item label={intl.get(`${prompt}.field.ApplicationType`).d('申请类型')}>
          {getFieldDecorator('applyType')(<CusInput allowClear/>)}
        </Form.Item>
      </Col>,
      <Col span={colSpan}>
      ...
      <Col span={colSpan}>
        <Form.Item label={intl.get(`${prompt}.field.request.date`).d('申请日期')}>
          {getFieldDecorator('applyDate')(<CusDatePicker style={{ width: '100%' }} />)}
        </Form.Item>
      </Col>,
      <Col span={colSpan}>
        <Form.Item label={intl.get(`${prompt}.field.ReasonOfBlackout`).d('列入黑名单原因')}>
          {getFieldDecorator('reason')(<CusInput allowClear/>)}
        </Form.Item>
      </Col>,
      <Col span={colSpan}>
