...
@connect(({ supplierHK, loading }) => ({
  ...
  saveLoading: loading.effects['supplierHK/updateInfoSave'],
...
export default class PurchaseUpdateInfo extends Component {
                    ...
                    customEvents: [
                      {
                        ...
                        func: () => {
                          // 业务逻辑
                          window.open(`/pub/spfm-hk/supplier/supplier-comparison?applyNumber=${applyNumber}`);
                        }
                      }
                    ]
                  }]
  ...
  async init() {
    ...
    if (id) {
      await dispatch({
        ...
        if (res) {
          ...
          const newCustomersDataSource = (res?.customers || []).map((item) => ({
            ...item,
            _status: 'update',
            id: uuid(),
          }))
          this.setState({
            ...
            customersDataSource: newCustomersDataSource,
    ...
    if (formRecordId !== undefined && formRecordId !== 'null') {
  ...
  queryUpdateInfoDetail() {
    const { location: { search }, dispatch } = this.props;
    const { applyNumber, formRecordId, state, permissionType } = queryString.parse(search.substring(1));
    dispatch({
      ...
      if (res) {
        ...
        this.handleContactChange(newContactPersonDataSource);
        ...
        this.platform.props.form.validateFields((err, values) => {
          if (!err) {
            ...
            const headEdit = {
              ...
              id: updateInfo?.editHead?.id, // 更新单id
              ...
              expireDate: dayjs.isDayjs(values.expireDate)
                ...
                : undefined,
              enableSupPortal: values.enableSupPortal,
              supModifyState: values.supModifyState,
              statusUpdateTime: values.statusUpdateTime,
              supplierVersions: values.supplierVersions,
              versionsUpdateTime: values.versionsUpdateTime,
            };
            const lineEdit = {
            ...
            const customer = customersDataSource?.map(i => {
              return {
  ...
  handleDelContactPersonData() {
    ...
    this.setState({
      ...
      contactRemove: contactPersonSelectedRows,
      contactPersonSelectedRowKeys: [],
      contactPersonSelectedRows: [],
    });
  }
  ...
  @Bind()
  handleAddClientData() {
    const { customersDataSource } = this.state;
    this.setState({
  ...
  @Bind()
  handleDelClientData() {
    const { customersDataSource = [], customersSelectRows = [] } = this.state;
    this.setState({
      customersDataSource: customersDataSource.filter(r => customersSelectRows.every(rd => rd.uuid !== r.uuid)),
      clientRemove: customersSelectRows,
      customersSelectRowKeys: [],
      customersSelectRows: []
    });
  }
  ...
  handleClientInfoRowSelectionChange(keys, rows) {
    this.setState({
      customersSelectRowKeys: keys,
      customersSelectRows: rows,
    })
  }
  ...
  render() {
    ...
    const headFormProps = {
      ...
      disabled: disabled || !(userList.realName == updateInfo?.editHead?.applyUser)
    };
    ...
    const contactTableProps = {
      ...
      readOnly: disabled || !(userList.realName == updateInfo?.editHead?.applyUser)
    };
    const clientTableProps = {
      dataSource: customersDataSource,
      otherDataSource: customersDataSource,
      rowSelection: {
        selectedRowKeys: customersSelectRowKeys,
    ...
    return (
      <PageWrapper loading={saveLoading}>
        <Collapse
        ...
        >
          ...
          <Panel
            ...
            header={
              <PanelHeader
                ...
                buttons={
                  <>
                    {!disabled && (userList.realName == updateInfo?.editHead?.applyUser) && <CusButton mini
                                onClick={this.handleDelContactPersonData}>{intl.get('hzero.common.button.delete').d('删除')}</CusButton>}
                    {!disabled && (userList.realName == updateInfo?.editHead?.applyUser) && <CusButton mini type="primary"
          ...
          <Panel
            ...
            key='attachment'
          >
            <p>{intl.get(`${prompt}.view.title.attachment.tips`).d('请上传商业登记证、供应商信息登记表、资质证照或其他辅助材料（如有)')}</p>
