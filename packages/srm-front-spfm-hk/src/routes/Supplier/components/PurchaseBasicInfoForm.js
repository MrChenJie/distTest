...
export default class PurchaseBasicInfoForm extends PureComponent {
  constructor(props) {
    super(props);
    props.onRef(this);
    this.state = {
  ...
  render() {
    const { form: { getFieldDecorator, getFieldValue }, initialValues, disabled = false, enumMap } = this.props;
    ...
    const { supplierStatus } = enumMap;
    const supplierStatusOptions = supplierStatus?.filter(i => i.value !== 'BLACKLIST');
    return (
          ...
          <Col {...gridSpan}>
            <Form.Item label={intl.get(`${prompt}.field.business.nature`).d('业务性质')}>
              {getFieldDecorator('professional', {
                ...
                <CusInput
                  placeholder={intl.get(`${prompt}.field.business.nature.placeholder`).d('请选择业务性质')}
                  disabled={disabled}
                />
              )}
            </Form.Item>
