...
export default class HeaderForm extends PureComponent {
  ...
  render() {
    const { form: {getFieldDecorator}, initialValues, disabled = false, currentUser } = this.props;
    const gridSpan = {
      xs: 12,
      sm: 12,
      md: 12,
          ...
          <Row>
            ...
            <Col {...gridSpan}>
              <Form.Item label={intl.get(`${prompt}.field.request.status`).d('申请状态')}>
                ...
                })(<CusSelect disabled lovCode="HKSP.APPLICATION_STATUS"/>)}
              </Form.Item>
            </Col>
            <Col {...gridSpan}>
            ...
            <Col {...gridSpan}>
              <Form.Item label={intl.get(`${prompt}.view.table.applicant`).d('申请人')}>
                {getFieldDecorator('applyUser', {
                  initialValue: initialValues?.applyUser || currentUser?.realName
                })(<CusInput disabled/>)}
              </Form.Item>
            </Col>
            <Col {...gridSpan}>
