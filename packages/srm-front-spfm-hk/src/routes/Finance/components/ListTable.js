...
export default class ApplyListTable extends React.PureComponent {
  ...
  pushRouter(record) {
    ...
    const { applyNumber, applyStatus, id, caseId, accessSource } = record;
    ...
    if (accessSource === 'portal') {
      if (caseId) {
        ...
        window.open(`${process.env.APPROVAL_PROCESS}/main/child-frame/app-approval/quick/process?templateCode=BPM-GYSCWXXBG-MH&pcThirdContentPageUrl=${encodeURIComponent(`${process.env.CMHK_LOGIN}/oauth/public/cmi/sso/login?redirect_uri=/pub/spfm-hk/supplier/PRSA-infomation-portal/bankUpdate&createdBy=${id}`)}`);
      }
      return;
    }
    // 根据审批状态跳转页面
    if (applyStatus === 'Draft') {
      if (caseId) {
        ...
        window.open(`${process.env.APPROVAL_PROCESS}/main/child-frame/app-approval/quick/process?templateCode=BMP-GYSCWXXBG&pcThirdContentPageUrl=${encodeURIComponent(`${process.env.CMHK_LOGIN}/oauth/public/cmi/sso/login?redirect_uri=/pub/spfm-hk/supplier/finance-draft?applyNumber=${id}`)}`);
      }
    }
    if (applyStatus === 'Inapproval' || applyStatus === 'Approved') {
      window.open(`${process.env.APPROVAL_PROCESS}/main/child-frame/app-approval/detail/share?detailType=SHARE&caseId=${caseId}`);
    }
  }
