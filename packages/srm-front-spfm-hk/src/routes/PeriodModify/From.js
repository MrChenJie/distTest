...
export default class PurchasePlanForm extends Component {
    ...
    render() {
        ...
        <Col {...gridSpan}>
            <Form.Item
                label={intl.get(`spfmhk.supplier.view.table.SupVerson.After`).d('供应商版本（变更后）')}
                wrapperCol={{ span: 24 }}
        ...
        <Col {...gridSpan}>
        <Form.Item
            label={intl.get(`spfmhk.supplier.view.table.changeOrderNumber`).d('变更单号')}
            wrapperCol={{ span: 24 }}
            name="applyNumber"
        >
            <Input />
        ...
        <Col {...gridSpan}>
            <Form.Item
                ...
                wrapperCol={{ span: 24 }}
                name="versionsUpdateStartTime"
            >
                <CusDatePicker
                    format={getDateFormat()}
                    disabledDate={(currentDate) =>
                        dayjs.isDayjs(this.baseForm?.current?.getFieldValue('versionsUpdateEndTime')) &&
                        currentDate &&
                        dayjs(currentDate).isAfter(
                            this.baseForm?.current?.getFieldValue('versionsUpdateEndTime')
                        )
                    }
                />
        ...
        <Col {...gridSpan}>
            <Form.Item
                ...
                wrapperCol={{ span: 24 }}
                name="versionsUpdateEndTime"
            >
                <CusDatePicker
                    format={getDateFormat()}
                    disabledDate={(currentDate) =>
                        dayjs.isDayjs(this.baseForm?.current?.getFieldValue('versionsUpdateStartTime')) &&
                        currentDate &&
                        currentDate.isBefore(
                            this.baseForm?.current?.getFieldValue('versionsUpdateStartTime')
                        )
                    }
                />
            </Form.Item>
        ...
        <Col {...gridSpan}>
            <Form.Item
                label={intl.get(`spfmhk.supplier.view.table.modifyReason`).d('修改原因')}
                wrapperCol={{ span: 24 }}
                name="editReason"
            >
                <Input />
