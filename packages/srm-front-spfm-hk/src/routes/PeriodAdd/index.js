...
@fastCodeLoader([
  'HKSP.REASON.REGISTER', //入围原因
])
@connect(({ loading = {}, supplierReport = {} }) => ({
  supplierReport,
  resultLoading: loading.effects['supplierReport/getPeriodAddInfo'],
}))

export default class PeriodAdd extends Component {
  constructor(props) {
    super(props);
    this.state = {
      searchForm: {}, // 查询条件
  ...
  componentDidMount() {
    ...
    // dispatch({
    //   type: 'supplierReport/getPeriodAddInfo',
    // });
  ...
  @Bind()
  formatValues(val) {
    const { supplierAccessStartTime, supplierAccessEndTime } = val || {};
    return {
      ...val,
      supplierAccessStartTime: supplierAccessStartTime ? dayjs(supplierAccessStartTime).format('YYYY-MM-DD hh:mm:ss') : undefined,
      supplierAccessEndTime: supplierAccessEndTime ? dayjs(supplierAccessEndTime).format('YYYY-MM-DD hh:mm:ss') : undefined,
    };
  }
  ...
  render() {
    ...
    const purchaseDataExportProps = {
      requestUrl: `/cmhk-supplier/v1/${organizationId}/cmhk-supplier-report/period-add/export`,
      method: "GET",
      downloadType: "Blob",
