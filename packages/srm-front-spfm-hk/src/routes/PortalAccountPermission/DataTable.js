...
class DataTable extends PureComponent {
  ...
  companyNameOnChange(_, lovRecord, record) {
    ...
    this.tableForm?.current?.setFieldsValue({
      ...
      [`${record[ROW_KEY]}#companyEmail`]: lovRecord.email,
    });
    ...
    const columns = [
      ...
      },
      {
        ...
        render: (val, record) => {
          ...
          //       ]}
          //     >
          //       <Input disabled onChange={(e) => this.emailOnChange(e.target.value, record)} />
          //     </Form.Item>
          //   </div>
          // ) : (
          //   tooltipRender(val)
