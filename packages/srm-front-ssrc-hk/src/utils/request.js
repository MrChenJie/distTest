...
import { API_HOST, AUTH_SELF_URL, LOGIN_URL, HZERO_OAUTH } from 'hzero-front/lib/utils/config';
import {
    ...
    if (status === 401) {
      ...
      if (accessToken) {
        request(`${HZERO_OAUTH}/public/token/kickoff`, {
          ...
          if (res.kickoff === 1) {
            ...
            !(
              window.location.href.indexOf('open=fs') > -1 ||
      ...
      if (newUrl.indexOf(AUTH_SELF_URL) !== -1) {
        ...
        if (
          ...
          window.location.href = `${CMI_HOST}/gwapi/oauth/public/cmi/sso/login?redirect_uri=${window.location.pathname}${window.location.search}`;
          setSession('isErrorFlag', false);
        } else {
