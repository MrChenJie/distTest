...
export async function sureContract(params) {
  ...
  return request(
    `${SRM_SPCM}/v1/${organizationId}/purchase-contract/change-status?pcHeaderStatus=TERMINATION`,
    {
      method: 'PUT',
      body: pcHeaderList,
    }
...
export async function sureRejectContract(params) {
  const { pcHeaderList } = params;
  return request(
    `${SRM_SPCM}/v1/${organizationId}/purchase-contract/change-status?pcHeaderStatus=CONFIRMED`,
    {
      method: 'PUT',
      body: pcHeaderList,
    }
...
export async function fetchHeader(params) {
  const { pcHeaderId, companyId, customizeUnitCode } = params;
