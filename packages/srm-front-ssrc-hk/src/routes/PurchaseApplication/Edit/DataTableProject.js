...
export default class DataTableProject extends React.Component {
  ...
  render() {
    ...
    const columns = [
      {
        title: tooltipRender(intl.get(`${promptCode}.model.label`).d('项目名称')),
      ...
      {
        title: tooltipRender(intl.get(`${promptCode}.model.label`).d('预算编号')),
      ...
      {
        title: tooltipRender(intl.get(`${promptCode}.model.label`).d('预算项目内容')),
        dataIndex: 'enquiryPriceStatus',
        width: 100,
        render: (_, record) => tooltipRender(record.enquiryPriceStatusMeaning),
      },
      {
        title: tooltipRender(intl.get(`${promptCode}`).d('成本中心')),
      ...
      {
        title: tooltipRender(intl.get(`${promptCode}.model.label`).d('业务活动')),
      ...
      {
        title: tooltipRender(intl.get(`${promptCode}.model.label`).d('剩余年度预算金额(HKD)')),
