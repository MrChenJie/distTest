...
@connect(({ loading = {}, purchaseApplicationModel = {} }) => ({
...
export default class UploadButton extends React.Component {
  ...
  onChange({ file, fileList }) {
    ...
    changeButton(this.state.closeLoading);
  ...
  onUploadSuccess(file, fileList) {
    const { onUploadSuccess, changeButton, dispatch } = this.props;
    this.setState({
      closeLoading: false,
    });
    changeButton(this.state.closeLoading);
    ...
    let uuid = fileList.map((item) => {
      // console.log(item,'???');
      return item.attachmentUuid;
    });
    console.log('数据uuid', uuid);
  ...
  async deleteFile(file) {
    const { onRemove, dispatch } = this.props;
    console.log(this.props, ')_)_)_)_)_)_)_)');
    const { fileList } = this.state;
    remove(fileList, (n) => n.uid === file.uid);
    if (file.url) {
      if (onRemove) {
  ...
  render() {
    ...
    let commonElements = delSelectedRows.filter((item2) => {
      const matchingElement = finalFileList.find((item) => item.url === item2.url);
      return matchingElement !== undefined;
    });
    let fileListRemaining = finalFileList.filter((item) => {
      return !commonElements.some((item2) => item2.url === item.url);
    });
    // console.log(fileListRemaining, 'fileListRemaining');
    ...
    return (
      <>
        ...
        <CusTable columns={columns} dataSource={fileListRemaining} />
        <CusFileViewer
