...
export default class purchaseApplication extends React.Component {
  ...
  handleDetele() {
    ...
    const deleteFlag = selectedRows.every((item) => item.prStatus === 'PENDING_REFER' && item.applicantUserId == currentUser.id);
  ...
  handleSavePurchaseType() {
    ...
    const bpmDraftUrl = `${process.env.APPROVAL_PROCESS}/main/child-frame/app-approval/quick/process`;
    ...
    this.modalForm?.current?.validateFields().then((values) => {
      ...
      if (modalFormValue.prType === 'frameworkSubOrder') {
        const templateCode = 'BPM_SCM_KJZDDCGSQ';
        // 跳转框架子订单页面
        const url = `${bpmDraftUrl}?templateCode=${templateCode}&pcThirdContentPageUrl=${encodeURIComponent(
      ...
      } else if(['salesbusinessproduct'].includes(modalFormValue.prType)) {
        const templateCode = 'BPM_SCM_SJJYWYPSP';
        // 预算类型是手机及业务用品
        window.open(
          `${bpmDraftUrl}?templateCode=${templateCode}&pcThirdContentPageUrl=${encodeURIComponent(
            `${process.env.CMHK_LOGIN}/oauth/public/cmi/sso/login?redirect_uri=/pub/ssrc-hk/phoneBusiness/Detail?related=2&prType=${this.state.prType}`
          )}`,
          '_blank'
        );
        // window.open(`/pub/ssrc-hk/phoneBusiness/Detail?related=2&prType=${this.state.prType}`, '_blank')
      } else if(['consignment', 'freeofcharge'].includes(modalFormValue.prType)) {
        ...
        const templateCode = 'BPM_SCM_jianyixunjiacaigoushenqing';
        window.open(
          `${bpmDraftUrl}?templateCode=${templateCode}&pcThirdContentPageUrl=${encodeURIComponent(
          ...
          // `/pub/ssrc-hk/purchaseApplication/edit?related=${relatedNumber}&prType=${this.state.prType}`,
          '_blank'
        );
      }
    })
              ...
              header={
                <PanelHeader
                  ...
                  buttons={
                    <>
                      <CusExcelExport
                        requestUrl={`/cmhk-pr-center/v1/${organizationId}/pr-apply-detail-heads/export`}
                        otherButtonProps={{
                          mini: true,
                        }}
                        buttonText={intl.get('hzero.common.button.export').d('导出')}
                        downloadType="Blob"
                        fileName={intl.get(`hzero.common.view.title.Resulttable`).d('结果展示')}
                        queryParams={() => {
                          return {
                            lang: currentUser.language,
                            ...this.getQueryParams(),
                ...
                >
                  <CusSelect
                    ...
                    onChange={(_, option) => {
                      ...
                      this.modalForm?.current?.resetFields(['relatedToProject'])
                    }}
                  />
                </Form.Item>
              </Col>
            </Row>
            {!['salesbusinessproduct', 'consignment', 'freeofcharge'].includes(this.modalForm?.current?.getFieldValue('prType')) && <Row>
              <Col>
                <Form.Item
                ...
                >
                  <CusSelect
                    ...
                    allowClear
                    options={
                      // idpValueMap['HKPC.RELATEDTOPROJECT']
                      ['simpleInquiry1', 'generalProcurement1', 'generalProcurement2', 'generalProcurement3', 'frameworkSubOrder'].includes(this.modalForm?.current?.getFieldValue('prType')) ?
                      filter(idpValueMap['HKPC.RELATEDTOPROJECT'], (item) =>
                        ['yes', 'no'].includes(item.value)
                      )
                      :
                      filter(idpValueMap['HKPC.RELATEDTOPROJECT'], (item) =>
                        ['yes', 'no', 'inventory'].includes(item.value)
                      )
                    }
                    onChange={() => {
                      console.log('this.modalForm', this.modalForm.current.getFieldValue('prType'))
                    }}
                  />
                </Form.Item>
              </Col>
