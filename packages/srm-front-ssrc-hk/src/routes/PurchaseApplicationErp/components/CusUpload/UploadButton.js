...
export default class UploadButton extends React.Component {
  ...
  onUploadSuccess(file, fileList) {
    const { onUploadSuccess, changeButton = (e) => e } = this.props;
    this.setState({
      closeLoading: false,
    });
    changeButton(this.state.closeLoading);
  ...
  changeFileList(fileList) {
    console.log(fileList, 'fileList666666');
    const {
      bucketName,
      bucketDirectory,
      tenantId,
  ...
  render() {
    ...
    console.log(delSelectedRows, 'delSelectedRows');
    let changedFileList = this.changeFileList(fileList);
    const finalFileList =
      this.state.fileList.length > 0
        ? this.state.fileList || changedFileList || []
        : changedFileList || [];
    console.log(finalFileList, 'finalFileList555');
    ...
    console.log(fileListRemaining, 'fileListRemaining666');
    const headers = {};
    if (accessToken) {
