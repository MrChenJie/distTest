...
export default class purchaseInquiryQuery extends React.Component {
  ...
  init() {
    ...
    const { related, id, formRecordId, prType } = queryString.parse(search.substring(1));
  ...
  handleSearchUnitName() {
    ...
    const { prStatus } = purchaseApplicationModel;
    const { id, related, formRecordId } = queryString.parse(search.substring(1));
    dispatch({
  ...
  handleSearch(resId, page = {}) {
    ...
    const { id, formRecordId, idErp } = queryString.parse(search.substring(1));
    dispatch({
      type: 'purchaseApplicationModel/getMaterialsDetail',
      payload: {
        page,
        refHeadId: resId || id || formRecordId || idErp,
      },
    }).then((res2) => {
      if (res2) {
        ...
        dispatch({
          ...
          payload: {
            id: resId || id || formRecordId || idErp,
          },
        }).then((res) => {
          if (res) {
            console.log(res, 'res');
            ...
            dispatch({
              ...
              payload: {
                ...
                procurementType: res?.procurementType, // 采购方式
              },
            });
            this.setState({
              contentObj: res,
  ...
  render() {
    ...
    const { purchaseApplicationLineSource, allDetailsInfo, prStatus, related, prType, projectType } =
      purchaseApplicationModel;
    ...
    const searchFormProps = {
      ...
      procurementType: contentObj?.procurementType, // 采购方式
    ...
    return (
      <div className={classnames(styles['out-pageWrapper'])} style={{ display: 'flex', flexDirection: 'column' }}>
