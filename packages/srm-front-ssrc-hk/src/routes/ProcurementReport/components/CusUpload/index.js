...
export default class Upload extends PureComponent {
  ...
  render() {
    const {
      ...
      hasTemplate,
      multiple = true,
      btnText = intl.get('HKPC.commom.view.button.upload').d('上传'),
      description,
      onChange,
      tenantId,
      btnProps = {},
    ...
    const actionPathname = isTenantRoleLevel()
      ? `${HZERO_FILE}/v1/${getCurrentOrganizationId()}/files/attachment/${
          isEncrypt ? 'encrypt-multipart' : 'multipart'
        }`
      : `${HZERO_FILE}/v1/files/attachment/multipart`;
    const action = `${actionPathname}`;
    const filesNumberWidth =
      showFilesNumber && ((filesNumber && filesNumber !== 0) || fileList.length > 0) ? 24 : 0;
        ...
        <div style={{ display: loading ? 'none' : '', overflow: 'hidden' }}>
          ...
          <CusSpin spinning={modalLoading}>
            <UploadButton
              ...
              getfileList={this.props.getfileList.bind(this)}
            />
          </CusSpin>
        </div>
      </React.Fragment>
    ...
    return (
      <React.Fragment>
        ...
        <CusModal
          ...
          width={610}
          footer={
            <CusButton onClick={this.closeUploadModal} loading={closeLoading}>
