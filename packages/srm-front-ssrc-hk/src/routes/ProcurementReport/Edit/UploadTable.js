...
import {getAccessToken, getCurrentOrganizationId, tableScrollWidth} from 'utils/utils';
...
export default class UploadTable extends React.Component {
  ...
  componentDidMount() {
    if (this.props.prApplyAttachmentList == null) {
      ...
      this.setState({
        prApplyAttachmentListValue: [
          ...this.props.prApplyAttachmentList,
    ...
    if (record.filePath) {
      request(`${HZERO_FILE}/v1/${getCurrentOrganizationId()}/files/download`, {
        ...
        query: {
          ...
          url: encodeURIComponent(record.filePath),
        ...
        if (res) {
          ...
          if ('msSaveOrOpenBlob' in navigator) {
            // 使用ie下载
            navigator.msSaveOrOpenBlob(blob, `Partner SOA.${extension}`);
            return false;
          }
          const url = URL.createObjectURL(blob);
          const a = document.createElement('a');
          a.href = url;
          a.download = `${record.name}.${extension}`;
          a.click();
          a.remove();
  ...
  render() {
    ...
    const {fileSource} = purchaseApplicationModel;
    const {
      selectedRows,
      selectedRowKeys,
      rfqResponseVisible,
        ...
        render: (val, record) => {
          return (
            <span className="action-link">
              <a style={{color: '#3271FE'}} onClick={() => this.handlePreviewFile(record)}>
                {val}
              </a>
            </span>
          );
    ...
    const rowSelection = {
      ...
      getCheckboxProps: (record) => ({
        disabled: prStatus != 'PENDING_REFER' && prStatus != '', // 选择框的是否可选
      }),
    };
    return (
      <React.Fragment>
