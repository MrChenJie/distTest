...
export default class DataTable extends React.Component {
  ...
  render() {
    ...
    const columns = [
      {
        title: intl.get(`${promptCode}.model.label`).d('文件标题'),
        dataIndex: 'title',
        width: 160,
        render: (val, record) => {
          return (
      ...
      {
        title: intl.get(`${promptCode}.model.label`).d('前处理人'),
        dataIndex: 'processor',
        width: 220,
        render: tooltipRender,
      },
      {
        title: intl.get(`${promptCode}.model.label`).d('当前状态'),
        dataIndex: 'currentStatus',
        width: 120,
        render: (_, record) => tooltipRender(record.enquiryPriceStatusMeaning),
      },
      {
        title: intl.get(`${promptCode}.model.label`).d('收到时间'),
