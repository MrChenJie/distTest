...
@connect(({ purchaseResultModel, loading }) => ({
  purchaseResultModel,
  fetchLoading: loading.effects['purchaseResultModel/queryPurchaseResultList'],
}))
@fastCodeLoader([
  ...
  'HKPC.DECISIONHIERARCHY',
])
export default class purchaseResult extends React.Component {
  ...
  getQueryParams() {
    ...
    return {
      ...
      enquiryEndDateTo: dayjs.isDayjs(enquiryEndDateTo)
        ...
        : undefined,
      dateFrom: dayjs.isDayjs(dateFrom) ? dateFrom.format('YYYY-MM-DD') : undefined,
      dateTo: dayjs.isDayjs(dateTo) ? dateTo.format('YYYY-MM-DD') : undefined,
    };
  }
  ...
  handleSubmitToApproval(data = []) {
    ...
    if (flag) {
      CusNotification.error({
        message: intl
          .get(`${promptCode}.warning.tips.onlySubmitYouself`)
          .d('仅可提交录入员为您本人的询价单！'),
      });
      return 0;
    }
    ...
    if (submitFlag) {
      if (businessType === 'ICTS') {
        CusModal.confirm({
          content: intl
            .get(`${promptCode}.view.config.isSubmitTickRfq`)
            .d('请确认是否提交勾选的询价单'),
          onOk: () => {
            dispatch({
        ...
        if (data.every((item) => item.subSoId)) {
          ...
          CusNotification.error({
            message: intl
              .get(`${promptCode}.tips.pleaseOtherSubmitRfq`)
              .d('请将有意向单和无意向单的询价单分别提交'),
          });
        }
      ...
      CusNotification.error({
        message: intl
          .get(`${promptCode}.tips.onlySubmitSameType`)
          .d('只能批量提交同一产品类型的询价单!'),
      });
    }
  }
  ...
  handlePublish(data = []) {
    ...
    if (publishFlag) {
      if (businessType === 'CHINA_DIA') {
        ...
        dispatch({
          type:
            businessType === 'ICTS' ? 'resaleRfq/ictsPublishSummary' : 'resaleRfq/publishSummary',
          payload: data.map((item) => {
            return {
              enquiryPriceId: item.enquiryPriceId,
              enquiryPriceRoundsId: item.enquiryPriceRoundsId,
      ...
      CusNotification.error({
        message: intl
          .get(`${promptCode}.tips.onlyPublishSameType`)
          .d('只能批量发布同一产品类型的询价单!'),
      });
    }
  }
