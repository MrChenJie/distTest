...
export default class DataTable extends React.Component {
  ...
  openPriceEntryDetail(record) {
    ...
    // dispatch(
    //   routerRedux.push({
    //     pathname: `/ssrc-hk/purchaseResult/edit`,
    ...
    //   })
    // );
    if (record.estimatedBudgetAmountHkd <= 500000) {
      window.open(
        `${process.env.APPROVAL_PROCESS}/main/child-frame/app-approval/quick/process?templateCode=BPM_SCM_yibancaigoujieguo&pcThirdContentPageUrl=${encodeURIComponent(
          `${process.env.CMHK_LOGIN}/oauth/public/cmi/sso/login?redirect_uri=/pub/ssrc-hk/purchaseResult/edit?${querystring.stringify(
      ...
      window.open(
        `${process.env.APPROVAL_PROCESS}/main/child-frame/app-approval/quick/process?templateCode=BPM_SCM_YBCGJG&pcThirdContentPageUrl=${encodeURIComponent(
  ...
  render() {
    ...
    const columns = [
        ...
        render: (_, record) => {
          return (
            <span style={{ display: 'block', textAlign: 'right' }}>
              {record.estimatedBudgetAmountHkd}
            </span>
          );
        },
      },
      {
        ...
        align: 'left',
        render: (_, record) => {
          return <span style={{ display: 'block', textAlign: 'right' }}>{record.paAmountHKD}</span>;
        },
      },
      {
