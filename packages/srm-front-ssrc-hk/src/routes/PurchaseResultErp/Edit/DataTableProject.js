...
export default class DataTableProject extends React.Component {
  ...
  render() {
    ...
    const columns = [
      {
        title: intl.get(`${promptCode}.model.label`).d('项目名称'),
      ...
      {
        title: intl.get(`${promptCode}.model.label`).d('预算编号'),
      ...
      },
      {
        ...
        render: (_, record) => {
          return (
            <div>{tooltipRender(record.enquiryPriceStatusMeaning)}</div>
          )
        },
      },
      {
        title: intl.get(`${promptCode}`).d('成本中心'),
        dataIndex: 'enquiryStartDate',
        width: 170,
        render: tooltipRender
      },
      {
        title: intl.get(`${promptCode}.model.label`).d('业务活动'),
