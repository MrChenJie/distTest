...
export default class DataTable extends React.Component {
  ...
  openPriceEntryDetail(record) {
    ...
    const { prNumber, estimatedBudgetAmountHkd } = record;
    // let url = '';
    // url = `${isPub ? '/pub' : ''}${SRM_SSRC}/standard-rfq/detail/${enquiryPriceId}/${enquiryPriceRoundsId}`
    // window.open(url);
    // dispatch(
    //   routerRedux.push({
    //     pathname: `/ssrc-hk/purchaseResultNormal/edit`,
    ...
    window.open(
      `${process.env.APPROVAL_PROCESS}/main/child-frame/app-approval/quick/process?guid=${'1957072883459246277'}&formRecordCode=${
        record.prNumber
      }&pcThirdContentPageUrl=${encodeURIComponent(
        `${process.env.CMHK_LOGIN}/oauth/public/cmi/sso/login?redirect_uri=/pub/ssrc-hk/purchaseResultNormal/edit?${querystring.stringify(
          { proCode: record.prNumber }
        )}`
      )}`
    );
  }
  ...
  render() {
    ...
    const columns = [
      {
        title: intl.get(`${promptCode}.model.label`).d('采购结果编号'),
        dataIndex: 'paNumber',
        width: 160,
        render: (val, record) => {
          return (
      ...
      {
        title: intl.get(`${promptCode}.model.label`).d('采购结果名称'),
        dataIndex: 'paName',
        width: 220,
        render: tooltipRender,
      },
      {
        title: intl.get(`${promptCode}.model.label`).d('采购结果状态'),
        dataIndex: 'paStatusMeaning',
        width: 120,
        render: (_, record) => tooltipRender(record.paStatusMeaning),
      },
      {
        title: intl.get(`${promptCode}.model.label`).d('结果申请日期'),
        dataIndex: 'creationDate',
        width: 120,
        render: dateRender,
      },
      {
        title: intl.get(`${promptCode}.model.label`).d('结果审批完成日期'),
      ...
      {
        title: intl.get(`${promptCode}.model.label`).d('申请人部门'),
        dataIndex: 'cratedBy',
        width: 120,
        align: 'left',
      },
      {
        title: intl.get(`${promptCode}.model.label`).d('采购申请编号'),
        dataIndex: 'prNumber',
        width: 120,
        align: 'left',
      },
      {
        title: intl.get(`${promptCode}.model.label`).d('采购方案编号'),
        dataIndex: 'ppNumber',
        width: 120,
        align: 'left',
      },
      {
        title: intl.get(`${promptCode}.model.label`).d('采购方式'),
        dataIndex: 'procurementMethod',
        width: 120,
        align: 'left',
      },
      {
        title: intl.get(`${promptCode}.model.label`).d('预估总金额(HKD)'),
        dataIndex: 'estimatedBudgetAmountHkd',
        width: 300,
        align: 'left',
      },
      {
        title: intl.get(`${promptCode}.model.label`).d('采购结果金额(HKD)'),
        dataIndex: 'paAmountHKD',
        width: 300,
        align: 'left',
      },
      {
        title: intl.get(`${promptCode}.model.label`).d('会议类型'),
        dataIndex: 'meetingType',
        width: 100,
        align: 'left',
      },
      {
        title: intl.get(`${promptCode}.model.label`).d('会议日期'),
      ...
      {
        title: intl.get(`${promptCode}.model.label`).d('采购经办人'),
        dataIndex: 'meetingDate',
        width: 120,
        align: 'left',
        render: dateRender,
      ...
      {
        title: intl.get(`${promptCode}.model.label`).d('采购申请类型'),
        dataIndex: 'meetingDate',
        width: 120,
        align: 'left',
        render: dateRender,
      ...
      {
        title: intl.get(`${promptCode}.model.label`).d('中选供应商'),
    ...
    return (
      <React.Fragment>
        ...
        <CusModal
          ...
          destroyOnClose
        >
          <RfqResponse {...this.props} nowRecord={nowRecord} />
        </CusModal>
