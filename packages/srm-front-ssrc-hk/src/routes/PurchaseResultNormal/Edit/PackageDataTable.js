...
export default class PackageDataTable extends React.Component {
  ...
  render() {
    ...
    const { activityCode } = querystring.parse(location?.search.substring(1));
    // 采购员才能编辑
    const isEdit = activityCode === 'CGY01';
    const { selectedRows, selectedRowKeys, rfqResponseVisible, nowRecord } = this.state;
    const { fourthPackageList = [], paStatusYB, isShow } = purchaseResultModel;
    ...
    const columns = [
          ...
          //   width: 120,
          //   render: (_, record) => {
          //     return <span>{record?.bidResult == '1' ? '中标' : 'record?.bidResult'}</span>;
          //   },
          // },
          // {
