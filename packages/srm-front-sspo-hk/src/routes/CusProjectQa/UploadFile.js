   ...
   const columns = [
     {
       ...
       key: 'fileName',
       width: getCurrentLanguage() === 'zh_CN' ? 250 : 206,
       dataIndex: 'fileName',
       render: (_, record) => {
        return (
     ...
     },
     {
       ...
       key: 'operate',
       width: disabled ?getCurrentLanguage() === 'zh_CN' ? 63 : 102  :getCurrentLanguage() === 'zh_CN' ? 105 : 162,
       dataIndex: 'operate',
       render: (_, record) => (
         <>
           ...
           {!disabled && <CusButton type="plain" style={{ marginLeft: '16px' }} onClick={() => handleDelete(record)} disabled={disabled}>
