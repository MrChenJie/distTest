...
import { createPagination, tableScrollWidth, getCurrentOrganizationId, getAccessToken, getCurrentLanguage } from 'utils/utils';
  ...
  render() {
    ...
    const columns = [
      ...
      },
      {
        ...
        width: getCurrentLanguage() === 'zh_CN' ? 65 : 88,
        render: (val, record) => {
          return (
            <CusButton type='plain' onClick={() => this.handleDownload(record)}>{intl.get(`${hcommon}.button.download`).d('下载')}</CusButton>
          )
        }
      }
    ];
