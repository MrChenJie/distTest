  ...
  const columns = [
    {
      ...
      render: (text, record) => (
        <Tooltip title={record.fileName} overlayClassName="customize-tooltip" color={'#646A73'}>
          <span className={styles.fileFontColor} onClick={() => handlePreview(record)}>
            {record.fileName}
          </span>
        </Tooltip>
      ),
    },
    {
      ...
      dataIndex: 'operator',
      width: getCurrentLanguage() === 'zh_CN' ? 65 : 100,
      fixed: 'right',
      render: (_, record) => (
        <>
          ...
          {!disabled && (
            <CusButton type="plain" onClick={() => handleDelete(record)} disabled={disabled}>
