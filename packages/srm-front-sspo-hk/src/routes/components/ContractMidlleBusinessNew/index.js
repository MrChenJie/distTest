...
export default class ContractMidlle extends Component {
  ...
  componentDidMount() {
    ...
    eventBus.on('saveBusinessNew', this.save)

  }
  componentWillUnmount() {
    eventBus.off('savsaveBusinessNeweB', this.save)
  }
  ...
  getAnswerTable() {
    ...
    request(
      ...
      if (res) {
        if (res.isNeedAnswerBusiness !== undefined) {
          ...
          if (this.props.onChangeTabsFlag) this.props.onChangeTabsFlag()
        }
      }
    })
  }
  ...
  */
  ...
  save(id) {
    ...
    console.log('dataSource1', dataSource1, this.props)
    ...
    if (oldeProId == null || oldeProId === id) {
      ...
      console.log('datas', id, oldeProId);
      if(isYesAnswer == '') {
        notification.error({
          message: intl.get(`bid.bidcommon.view.message.Psao`).d('请选择线上应答')
        });
        if (activeKey == '3') eventBus.emit('callback', '3', true)
        return
      }
 ...
 */
  ...
  */
  ...
  render() {
    ...
    const columns = [
      {
        ...
        width: getCurrentLanguage() === 'zh_CN' ? 80 : 130,
        fixed: 'left',
        dataIndex: 'orderSeq',
      ...
      },
      {
        ...
        dataIndex: 'isLowestRequireMeaning',
        render: (_, record) => {
    ...
    return (
      <CusSpin spinning={submitContractLoading || saving || loading}>
