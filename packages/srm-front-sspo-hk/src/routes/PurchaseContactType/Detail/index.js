...
import { merge, isArray, isNumber, isUndefined, difference } from 'lodash';
import { connect } from 'dva';
import { DETAIL_DEFAULT_CLASSNAME, DATETIME_MIN, DETAIL_CARD_CLASSNAME } from 'utils/constants';
...
const commonPrompt = 'spcm.purchaseContractType.view.message';
...
export default class Detail extends Component {
  ...
  stageHandleDeleteLines(key) {
    ...
    Modal.confirm({
      title: intl.get(`${commonPrompt}.deletePurchaseLines`).d('是否删除'),
      onOk: () => {
  ...
  fetchTerms(page = {}) {
    ...
    if (pcTypeId) {
      dispatch({
        ...
        if (res) {
          this.setState({
            termTypeDataSource: res.content.map(n => ({ ...n, _status: 'update' })), // 业务条款数据
              ...
              ([
                ...
                const headerData = {
                  ...
                  startDateActive: startDateActive
                    ? startDateActive.format(DATETIME_MIN)
                    : undefined,
                  endDateActive: endDateActive ? endDateActive.format(DATETIME_MIN) : undefined,
                };
                ...
                const pcPartnerTypeDTOList = pcPartnerTypeDTOListFilter.filter(item => item.edited);
                ...
                const pcType = merge(headerInfo, headerData);
  ...
  render() {
    ...
    const { pcTypeId = headerInfo.pcHeaderId } = querystring.parse(search.substr(1));
    return (
      <Fragment>
        <Header
        ...
        >
          <Button loading={saving} onClick={this.save} icon="save" type="primary" disabled={saving}>
            ...
            <Row gutter={48}>
              <Col span={24}>
                <Card
                  ...
                  title={
                    <h3>{intl.get(`${commonPrompt}.basicInformation`).d('协议类型基础信息')}</h3>
                  }
                >
                ...
                >
                  {pcTypeId && (
                    <Collapse.Panel
                      ...
                      header={
                        <PanelHeader
                          title={intl.get(`${commonPrompt}.contractPartners`).d('合同伙伴类型定义')}
                  ...
                  {pcTypeId && (
                    <Collapse.Panel
                      ...
                      header={
                        <PanelHeader
                          title={intl.get(`${commonPrompt}.contractStageInfo`).d('协议阶段定义')}
                          collapseKeys={collapseKeys}
                  ...
                  {pcTypeId && (
                    <Collapse.Panel
                      ...
                      header={
                        <PanelHeader
                          title={intl.get(`${commonPrompt}.businessTerms`).d('业务条款定义')}
                          collapseKeys={collapseKeys}
                  ...
                  {pcTypeId && (
                    <Collapse.Panel
                      ...
                      header={
                        <PanelHeader
                          title={intl.get(`${commonPrompt}.attachmentDefinition`).d('附件类型定义')}
                          collapseKeys={collapseKeys}
