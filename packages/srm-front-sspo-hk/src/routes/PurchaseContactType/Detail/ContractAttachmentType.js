...
export default class ContractAttachmentType extends Component {
  ...
  getColumns() {
    ...
    const columnArray = [
      ...
      {
        title: intl.get(`spcm.purchaseContactType.model.attachmentNullableFlag`).d('是否必传'),
  ...
  render() {
    ...
    const rowSelection = {
      ...
      getCheckboxProps: record => ({
        disabled: record._status === 'update' && record[attachmentKey], // Column configuration not to be checked
      }),
    };
    const tableProps = {
      editContractType,
