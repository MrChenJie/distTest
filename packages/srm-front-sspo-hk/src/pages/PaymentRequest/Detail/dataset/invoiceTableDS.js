...
async function queryBillAmount(invoiceNum, vendorNum, ouOrgCode) {
  return request(
    ...
    },
    {
      ...
      validator: (value, name, record) => {
        ...
        if (record.get('instalmentFlag')) {
          ...
          if (typeof value === 'number' && value <= invoiceAmount) {
            return intl
  ...
  events: {
    update: ({ record, name, value, dataSet }) => {
      if (name === 'instalmentFlag' && value && record.get('invoiceNum')) {
        ...
        queryBillAmount(record.get('invoiceNum'), vendorCompanyNum, ouOrgCode).then((res) => {
