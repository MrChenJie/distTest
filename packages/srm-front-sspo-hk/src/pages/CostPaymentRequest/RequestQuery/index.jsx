...
export default class CostRequestQuery extends PureComponent {
  ...
  addRecord() {
    ...
    history.push({
      pathname: `${isPub ? '/pub' : ''}/spcm/cost/payment/request/detail/${-1}`,
      search: `${open ? '?open=fs' : ''}`,
    });
  }
    ...
    confirm({
      ...
      onOk() {
        ...
        if (currentSelected.length === 1) {
          request(
            `${SRM_SPUC}/v1/${getCurrentOrganizationId()}/cost-payment-requests/copy/${
  ...
  render() {
    const { costRequestLoading } = this.state;
    return (
      <Fragment>
