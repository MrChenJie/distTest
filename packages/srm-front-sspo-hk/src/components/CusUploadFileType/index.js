...
import { isEmpty, isFunction, partition } from 'lodash';
...
export default class Upload extends PureComponent {
  ...
  handleOK() {
    ...
    console.log('123', this.upload.state.fileList);
    const [commercialOrQuotationArray, otherArray] = partition(this.upload.state.fileList, item => 
      ['CommercialTender', 'QuotationFile'].includes(item.fileType)
    );

    console.log('commercialOrQuotationArray', commercialOrQuotationArray);
    console.log('otherArray', otherArray?.filter(item => item.newUuid));
    this.setState({
      isOkLoading: true,
    })
    // otherArray==8类文件
    if(otherArray.length > 0) {
      dispatch({
        type: 'contractMaintain/saveAttachmentTypeOther',
        payload: otherArray?.filter(item => item.newUuid).map(el => ({
            fileUrl: el.originalUrl,
            attachmentUuid: el.newUuid,
        }))
      }).then((res) => {
        if(res) {
          this.setState({
            ...
            this.handleBidCheckFile();
          })
        }
      })
    }

    // commercialOrQuotationArray==非8类文件
    if(commercialOrQuotationArray.length > 0) {
      dispatch({
        type: 'contractMaintain/saveAttachmentType',
        payload: commercialOrQuotationArray?.map(item => ({
          proId: item?.proId,
  ...
  @Bind()
  handleBidCheckFile() {
    const { dispatch, lineRecord } = this.props;
    console.log('lineRecord', lineRecord);
    
    dispatch({
      type: 'contractMaintain/getBidCheckFile',
      payload: {
        proId: lineRecord?.proId,
        milestoneId: lineRecord?.milestoneId,
      }
    }).then((res) => {
