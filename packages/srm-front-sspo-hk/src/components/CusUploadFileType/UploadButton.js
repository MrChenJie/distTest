...
import { Upload, Form } from 'hzero-ui';
...
export default class UploadButton extends React.Component {
  ...
  getUuidByFileType = (val, record) => {
    const { dispatch } = this.props;
    dispatch({
      type: 'contractMaintain/queryUuidByFileType',
      payload: {
        proId: record.proId,
  ...
  render() {
    ...
    const columns = [
      {
        title: intl.get(`HKPC.commom.view.title.filecategory`).d('分类'),
        width: 180,
        dataIndex: 'fileType',
        render: (_, record) => {
          return (
            <Form.Item>
              {record.$form.getFieldDecorator('fileType', {
                ...
                <CusSelect
                  options={portalAttachlist}
                  onChange={(val) => {
                    if(['CommercialTender', 'QuotationFile'].includes(val)) {
                      record.fileType = val;
                      this.setState({ })
                    } else {
                      this.getUuidByFileType(val, record);
                    }
                  }}
                />
              )}
      ...
      },
      {
        ...
        width: getCurrentLanguage() === 'zh_CN' ? (viewOnly ? 200 : 210) : viewOnly ? 200 : 210,
        dataIndex: 'name',
        render: (_, record) => {
          return (
            <a onClick={() => this.handlePreviewFile(record)}>
              {record.name}
            </a>
          )
        }
      },
      {
        ...
        width: getCurrentLanguage() === 'zh_CN' ? (viewOnly ? 220 : 230) : viewOnly ? 220 : 230,
        dataIndex: 'creationDate',
        render: (_, record) => {
          return (
            <div>
              {record.creationDate ? record.creationDate : dayjs().format('YYYY-MM-DD HH:mm:ss')}
            </div>
          )
        }
      },
      {
        ...
        width: getCurrentLanguage() === 'zh_CN' ? (viewOnly ? 98 : 130) : viewOnly ? 170 : 228,
        dataIndex: 'operation',
        render: (_, record) => {
    ...
    return (
      <>
        ...
        <EditTable columns={columns} dataSource={finalFileList} rowKey="rowKey" />
        <CusFileViewer
