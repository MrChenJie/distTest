...
export default class ParamsDrawer extends PureComponent {
  ...
  render() {
    ...
    const { getFieldDecorator, getFieldValue = e => e } = form;
    ...
    const isShowBodyDefaultValue =
      paramType === 'BODY' && (mimeType !== 'xml' && mimeType !== 'json' && mimeType !== 'raw');
    ...
    return (
      ...
      >
        <Form>
          ...
          {getFieldValue('paramValueType') === 'FILE' && (
            <FormItem
              label={intl.get(`hitf.services.model.services.defaultValue`).d('默认值')}
              {...MODAL_FORM_ITEM_LAYOUT}
            >
          ...
          {isShowBodyDefaultValue && (
            <FormItem
              label={
                getFieldValue('paramValueType') === 'FILE'
                  ? ''
                  : intl.get(`hitf.services.model.services.defaultValue`).d('默认值')
              }
              labelCol={{ span: getFieldValue('paramValueType') === 'FILE' ? 0 : 6 }}
              wrapperCol={{
                span: getFieldValue('paramValueType') === 'FILE' ? 0 : 16,
                offset: getFieldValue('paramValueType') === 'FILE' ? 7 : 0,
              }}
            >
              {getFieldDecorator('defaultValueLongtext', {
                rules: [
                  // {
                  //   required: getFieldValue('paramValueType') === 'FILE',
                  //   message: intl.get('hzero.common.validation.notNull', {
                  //     name: intl.get(`hitf.services.model.services.defaultValue`).d('默认值'),
                  //   }),
                  // },
              ...
              })(getFieldValue('paramValueType') === 'FILE' ? <div /> : <Input />)}
            </FormItem>
          )}
        </Form>
      </Modal>
