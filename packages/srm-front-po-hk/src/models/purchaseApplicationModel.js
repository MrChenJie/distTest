...
export default {
  ...
  effects: {
    ...
    *fetchDetailEnum(params, { put, call }) {
      const detailEnumMap = getResponse(
        yield call(queryMapIdpValue, {
          ...
          inviteSupplierOptions: 'HKPC.SUPPLIERTYPE', //邀请供应商类型
        })
      );
      if (detailEnumMap) {
        yield put({
    ...
    // 判断简易询价单据是否已存在
    *isRepeat({ payload }, { call, put }) {
      return cusGetResponse(yield call(isRepeat, payload));
    },
    // 保存简易询价
    ...
    *queryDetail({ payload }, { call, put }) {
    ...
    // 简易询价详情—— 采购申请进入
    *applyQueryDetail({ payload }, { call, put }) {
      return cusGetResponse(yield call(applyQueryDetail, payload));
    },
    // 查询项目id
    ...
    *saveAttachment({ payload }, { call, put }) {
    ...
    // 邀请供应商查询
    *checkSupplier({ payload }, { call, put }) {
      return cusGetResponse(yield call(checkSupplier, payload));
    },
    // 汇率查询
    ...
    *saveEditMat({ payload }, { call }) {
    ...
    // 查询分配的物料
    *getMatList({ payload }, { call }) {
      const res = cusGetResponse(yield call(getMatList, payload));
      return res;
    },
    // 价格汇总——物料保存
    *priceSummaryMatsave({ payload }, { call, put }) {
      const res = cusGetResponse(yield call(priceSummaryMatsave, payload));
      return res;
    },
    // 保存选择的供应商
    *saveSelectSupplier({ payload }, { call }) {
    ...
    // 里程碑使用的中选供应商
    *getSelectSuppplier({ payload }, { call }) {
      const res = cusGetResponse(yield call(getSelectSuppplier, payload));
      return res;
    },
    // 校验价格汇总表是否已申请
    *getCheckSend({ payload }, { call }) {
      const res = cusGetResponse(yield call(getCheckSend, payload));
      return res;
    },
    // 查询框架协议汇总表
    *getPriceSumlList({ payload }, { call }) {
      const res = cusGetResponse(yield call(getPriceSumlList, payload));
      return res;
    },
    // 查询录入报价供应商
    *getEnterQuotationList({ payload }, { call }) {
      const res = cusGetResponse(yield call(getEnterQuotationList, payload));
      return res;
    },
  },
