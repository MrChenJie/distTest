...
export default class index extends Component {
  ...
  handleQueryProgress = async () => {
    ...
    let res = await getPaymentProgress({ requestId, paymentType: 'COST_PAYMENT' });
    ...
    res = res.map((list, num) => {
      ...
      if (list.haveChildFlag === 'Y') {
        childPaymentProgressStatusList = list.childPaymentProgressStatusList.map((list2) => {
  ...
  handleDetailPanel = async (nodeStatus) => {
    ...
    const res = await getPaymentProgressDetail({
      costRequestType: 'RESALE_PAYMENT',
      nodeStatus,
      pressLevel,
      requestId,
    });
