...
export default ({ onDeleteSuccess = (e) => e, isLine = false }) => ({
  ...
  transport: {
    destroy: ({ data }) => {
      if (isLine) {
        return {
          url: `${SRM_SPUC}/v1/isp/${organizationId}/cost-invoice-filess`,
          method: 'DELETE',
          data,
        };
      };
