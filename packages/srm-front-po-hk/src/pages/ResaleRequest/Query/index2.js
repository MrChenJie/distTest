...
class ResaleRequest extends Component {
  ...
  handleAsyncExport = (fileDesc = undefined) => {
    ...
    const queryParams = this.getQueryParams();
    cusRequest(`/hrpt/v1/${getCurrentOrganizationId()}/payment/resale-export`, {
      method: 'GET',
      query: {
        ...queryParams,
        fileName: !isEmpty(fileDesc) ? `RESALE_${fileDesc}` : `RESALE_`,
        async: true,
        exportType: 'DATA',
      ...
      const { resetFields } = this.asyncExportForm?.current;
      resetFields();
      if (res && res.failed) {
  ...
  addRecord = () => {
    ...
    window.open(`${isPub ? '/pub' : ''}/spcm/payment-request-resale/create`);
  };
  ...
  render() {
    ...
    const { activeKey, isPub, selectedRowKeys, selectedRows, isView, spinning } = this.state;
