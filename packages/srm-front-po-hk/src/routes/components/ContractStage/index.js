...
@withCustomize({
  unitCode: ['SPCM.PURCHASE_CONTRACT_MAINTAIN.STAGE'],
})
export default class ContractStage extends Component {
  ...
  handleChangeCurrencyCode(type, lovRecord, record) {
    ...
    currencyCode === null ? setFieldsValue({ [type]: 'CYN' }) : '';
    console.log(getFieldValue(type));
    if (type === 'supplierCurrencyCode') {
      getFieldValue('purchaseCurrencyCode') === currencyCode ||
      (currencyCode === null && getFieldValue('purchaseCurrencyCode') === 'CYN')
        ? setFieldsValue({ exchangeRate: 1, supplierCurrencyCode: 'CYN' })
        : '';
    } else {
      getFieldValue('supplierCurrencyCode') === currencyCode ||
      (currencyCode === null && getFieldValue('supplierCurrencyCode') === 'CYN')
        ? setFieldsValue({ exchangeRate: 1, supplierCurrencyCode: 'CYN' })
        : '';
    }
  }
  ...
  handleItemOnChange(value, lovRecord, record) {
    const { onChangeListData, dataSource, onFetchCategory } = this.props;
    const {
      $form: { setFieldsValue, registerField },
    } = record;
    this.handleChangeFormItem();
  ...
  getColumns() {
    ...
    const columnArray = [
                ...
                rules: [
                  {
                    required: true,
                    message: intl.get('hzero.common.validation.notNull', {
                      name: intl.get(`spcm.common.exchangeRate`).d('汇率'),
                    }),
                  },
                ],
                initialValue:
      ...
      },
      {
        ...
        width: 150,
        render: val => this.numberRender(val, 2),
      },
      {
  ...
  render() {
    ...
    return (
      <Fragment>
        ...
        {customizeTable(
          {
            code: 'SPCM.PURCHASE_CONTRACT_MAINTAIN.STAGE',
          },
