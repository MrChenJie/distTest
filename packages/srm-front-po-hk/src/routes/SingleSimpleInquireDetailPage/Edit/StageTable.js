...
@connect(({ singlePurchaseApplicationModel, loading = {} }) => ({
  ...
  queryLoading: loading.effects['singlePurchaseApplicationModel/stageQuery'],
  sendLoading: loading.effects['singlePurchaseApplicationModel/supplierSend'],
  submitLoading: loading.effects['singlePurchaseApplicationModel/editTimeSave'] ||
  loading.effects['singlePurchaseApplicationModel/addNextRoundSave'],
}))
 ...
 */
  ...
  */
  ...
  handleStageQuery(refHeadId) {
    ...
    // 调用阶段查询接口；
    dispatch({
      type: `singlePurchaseApplicationModel/stageQuery`,
      payload: {
        refHeadId,  // 基本信息id
      }
    }).then(res => {
  ...
  */
  ...
  handleMilestoneState(record, callback) {
    const { dispatch } = this.props;
    dispatch({
      type: 'singlePurchaseApplicationModel/getMilestoneState',
      payload: {
  ...
  */
  ...
  openEditTimeModal(record, stage) {
    const { dispatch, singlePurchaseApplicationModel } = this.props;
    const { refHeadId, stageList } = singlePurchaseApplicationModel;
    let rounds = 0;
    if (stage === 'one') {
 ...
 */
  ...
  sendAllemail() {
    const { dispatch, singlePurchaseApplicationModel } = this.props;
    const { supplierList = [], refHeadId } = singlePurchaseApplicationModel;
    // 发送
    dispatch({
 ...
 */
  ...
  submitQuotation() {
    // debugger
    const { dispatch, singlePurchaseApplicationModel } = this.props;
    const { selectedRowKeys, refHeadId } = singlePurchaseApplicationModel;
    ...
    if (stage === 'one') {
      this.handleMilestoneState({ ...record }, (res) => {
        if (res) {
          ...
          // 提交接口
          dispatch({
            type: `singlePurchaseApplicationModel/editTimeSave`,
            payload: {
              ...handleQuotationForm,
            },
          }).then(() => {
      ...
      // 提交接口
      dispatch({
        type: `singlePurchaseApplicationModel/addNextRoundSave`,
        payload: {
          ...handleQuotationForm,
  ...
  handleDetailsPage(record) {
    ...
    const refPrFirstId = inquireBaseInfo?.prId;
    //采购申请名称 prName
    const prName = inquireBaseInfo?.prName;
    //采购申请编号 prNumber
    const prNumber = inquireBaseInfo?.prNumber;
    //项目编码 projectNumber
    const projectNumber = inquireBaseInfo?.projectNumber;
    //报价货币 currency
    const currency = inquireBaseInfo?.currency;
    //轮次 rounds
    const rounds = record.rounds;
    const isPub = location.pathname.includes('pub'); // 判断是否为pub页面
    let url = `${isPub ? '/pub' : ''}/ssrc-hk/single-interior/quotation-document-detail-page?id=${id}&refPrFirstId=${refPrFirstId}&projectNumber=${projectNumber}&currency=${currency}&rounds=${rounds}&prNumber=${prNumber}&prName=${prName}`;
  ...
  //   //获取跳转页面所需要带的参数
  //   const { singlePurchaseApplicationModel, caseId, } = this.props;
  //   const { inquireBaseInfo, stageList = [], equal } = singlePurchaseApplicationModel;
  //   //采购中心采购申请询价单主表 id 
  //   const id = inquireBaseInfo?.id;
  //   //关联采购申请ID refPrFirstId
  ...
  //   } else {
  //     window.open(`${process.env.APPROVAL_PROCESS}/main/child-frame/app-approval/quick/process?templateCode=BPM_SCM_YBCGXJ&pcThirdContentPageUrl=${encodeURIComponent(`${process.env.CMHK_LOGIN}/oauth/public/cmi/sso/login?redirect_uri=/pub/ssrc-hk/single-interior/price-summary-detail-page&refHeadId=${id}`)}`);
  //   }
  ...
  @Bind()
  changeCheck(record, index, supName) {
    const { dispatch, singlePurchaseApplicationModel } = this.props;
  ...
  handlePriceSummaryPage(record) {
    const { dispatch, singlePurchaseApplicationModel } = this.props;
    const { priceSummaryList = [], refHeadId, inquireBaseInfo } = singlePurchaseApplicationModel;
    dispatch({
  ...
  getFrameAgreementList() {
    ...
    const { refHeadId } = singlePurchaseApplicationModel;
    dispatch({
      type: 'singlePurchaseApplicationModel/getPriceSumlList',
      payload: {
        refHeadId,
      }
    }).then((res) => {
  ...
  getSelectSupplierList() {
    ...
    dispatch({
      type: `singlePurchaseApplicationModel/getSelectSuppplier`,
      payload: {
        id: refHeadId,
        refHeadId,
      },
  ...
  handleOk() {
    const { dispatch, singlePurchaseApplicationModel } = this.props;
    const { refHeadId, inquireBaseInfo } = singlePurchaseApplicationModel;
    const { selectData, supList, milestoneId } = this.state;
    const url = `${process.env.APPROVAL_PROCESS}/main/child-frame/app-approval/quick/process?templateCode=BPM_SCM_YBCGXJ&pcThirdContentPageUrl=${encodeURIComponent(`${process.env.CMHK_LOGIN}/oauth/public/cmi/sso/login?redirect_uri=/pub/ssrc-hk/single-interior/demander-confirmation-implement&refHeadId=${refHeadId}`)}`;
    ...
    if(inquireBaseInfo?.isFrameAgreement === 'Y') {
      ...
      dispatch({
        type: `singlePurchaseApplicationModel/saveSelectSupplier`,
        payload: {
          priceTolList: params,
          url,
          templateCode: 'BPM_SCM_YBCGXJ', // 询价2
          refHeadId,
          stageId: milestoneId,
        }
      }).then((res) => {
  ...
  handleSave() {
    const { dispatch, singlePurchaseApplicationModel } = this.props;
    const { refHeadId, allocationAmountDataSource } = singlePurchaseApplicationModel;
  ...
  handleEnterQuotation = (page = {}, record) => {
    ...
    const { dispatch } = this.props;
    dispatch({
      type: 'singlePurchaseApplicationModel/getEnterQuotationList',
      payload: {
        page,
        isTry: 'Y',
  ...
  render() {
    ...
    const { stageList = [], inquireBaseInfo = {}, supplierList = [], reviewPriceData = [], allocationAmountDataSource = [] } = singlePurchaseApplicationModel;
    const {
      stage,
      modalVisible,
      reviewModalVisible,
