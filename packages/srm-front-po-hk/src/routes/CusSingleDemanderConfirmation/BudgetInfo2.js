...
export default class BudgetInfo extends React.Component {
  ...
  budgetTypeOnChange(value, lovRecord, record) {
    ...
    const { $form } = record;
    const { budgetInfoList = [] } = singlePurchaseApplicationCusModel
    this.setState({
  ...
  //     singlePurchaseApplicationCusModel,
  //   } = this.props;
  //   const { purchasingCategoriesCode } = singlePurchaseApplicationCusModel;
    ...
    const columns = [
      ...
      ['0', '1'].includes(projectType) && {
        title: labelTip({
          ...
          tip: intl.get(`${promptCode}.view.title.businessactivity`).d('若预算类型为Opex，可不填业务活动'),
        }),
