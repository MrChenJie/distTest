...
@connect(({ purchaseApplicationModel, loading }) => ({
  ...
  fetchLoading: loading.effects['purchaseApplicationModel/getPriceCollect'] ||
    loading.effects['purchaseApplicationModel/getQuotationListDetail'] ||
    loading.effects['purchaseApplicationModel/getPriceTolList'],
}))
export default class PriceSummary extends Component {
  ...
  handleQuery() {
    ...
    // 查询报价文件详情数据
    dispatch({
      type: 'purchaseApplicationModel/getPriceCollect',
      payload: {
        id,
      }
    }).then(res => {
      if (res) {
        const priceBasicInfo = { ...res, numberRenderEstimatedHKD: numberRender(res?.estimatedHKD, 2) };
    ...
    // 查询价格汇总表数据
    dispatch({
      type: `purchaseApplicationModel/getPriceTolList`,
      payload: {
  ...
  save(callback) {
    const { dispatch, purchaseApplicationModel } = this.props;
    const { refHeadId, formRecordId } = this.state;
    const { priceBasicInfo = {}, priceSummaryList = [] } = purchaseApplicationModel;
    if (priceSummaryList.length <= 0) {
      CusNotification.error({
        message: intl.get(`${promptCode}.view.message.choosequotationprompt`).d('请选择供应商的报价')
      });
      return;
    }
    const id = refHeadId || formRecordId;
    const amount = priceSummaryList[0]?.priceHkd;
    ...
    dispatch({
      type: `purchaseApplicationModel/priceSummarySave`,
      payload: [{
        refHeadId,
        refSupId,
        rounds,
      ...
      if (res) {
        if (typeof callback === 'function') {
          callback({ refHeadId, refSupId, rounds, amount, equal, headId, id, xuqiuren, prName, shenqingbumen, shenqingren });
        }
      }
    })
  }
  ...
  handleComparePricebutton() {
    const { dispatch, purchaseApplicationModel } = this.props;
    const { refHeadId, formRecordId } = this.state;
    const { priceSummaryList = [], priceBasicInfo = {} } = purchaseApplicationModel;
      ...
      if (res) {
        ...
        res?.supResultVoList?.map((item, index) => {
          const obj = {
            ...
            render: (val, record) => {
              return (
                <Checkbox disabled={record?.data[index]?.priceTotal === null} checked={record?.data[index]?.seleted} onChange={() => this.changeCheck(record, index)}>
                  {record?.data[index]?.priceTotal === null ? intl.get(`${promptCode}.view.title.notsubmitquotation`).d('未报价') : numberRender(record?.data[index]?.priceTotal, 2)}
                </Checkbox>
              )
            }
          };
        ...
        if (priceSummaryList && priceSummaryList.length > 0) {
          dataSource.map(item => {
            return item.data.map(i => {
              if (item.roundNum === priceSummaryList[0].rounds && i.refSupId === priceSummaryList[0].refSupId) {
                i.seleted = true;
              }
            })
          })
  ...
  handleSave() {
    ...
    const id = refHeadId || formRecordId;
    dispatch({
      type: `purchaseApplicationModel/getPriceSummaryList`,
      payload: [{
        refHeadId: parseInt(id),
        refSupId: parseInt(selectData?.refSupId),
        rounds: selectData?.roundNum,
      }]
    }).then(res => {
      if (res) {
        dispatch({
  ...
  handleOkMat = () => {
    ...
    if(params.length > 0) {
      dispatch({
        type: 'purchaseApplicationModel/saveEditMat',
        payload: params
      }).then((res) => {
        if(res) {
          this.setState({
  ...
  render() {
    const { purchaseApplicationModel = [], fetchLoading = false, form} = this.props;
    const { activeKey, modalVisible, selectedRowKeys, columns, state, addMatModalVisible } = this.state;
    const { reviewPriceData = [], priceBasicInfo = {}, priceSummaryList = [] } = purchaseApplicationModel
    const FormProps = {
      priceBasicInfo,
      onRef: (ref) => {
