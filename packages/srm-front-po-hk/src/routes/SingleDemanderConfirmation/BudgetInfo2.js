...
export default class BudgetInfo extends React.Component {
  ...
  budgetTypeOnChange(value, lovRecord, record) {
    ...
    const { $form } = record;
    const { budgetInfoList = [] } = singlePurchaseApplicationModel
    this.setState({
  ...
  changePurchasingCategories(e, a,record) {
    ...
    // const { purchasingCategoriesVal } = singlePurchaseApplicationModel;
    if (e) {
      dispatch({
        type: `singlePurchaseApplicationModel/updateState`,
        payload: {
  ...
  //     singlePurchaseApplicationModel,
  //   } = this.props;
  //   const { purchasingCategoriesCode } = singlePurchaseApplicationModel;
  ...
  render() {
    ...
    //       return (
    //         <Form.Item>
    //           {record.$form.getFieldDecorator(`budgetType${record.uuid}`, {
    ...
    //           //   code="CMHK.API.C/O"
    //           //   queryParams={{ projectNum: projectId }}
    ...
    //             queryParams={{ projectCode: projectId }}
    ...
    //     required: true,
    //     render: (_, record) => {
    //       console.log('物料编码', record);
    //       return this.props.singlePurchaseApplicationModel.purchasingCategories ? (
    //         <Form.Item>
    ...
    //             queryParams={{ purchasingCategory: this.props.singlePurchaseApplicationModel.purchasingCategories }}
    ...
    //     required: true,
    //     render: (_, record) => {
    //       console.log('业务活动', record.budgetType)
    //       return (
    ...
    //   },
    //   {
    //     title: intl.get(`${promptCode}.view.title.purchaseamount`).d('采购申请金额（HKD）'),
    ...
    //       return (
    //         <Form.Item>
    //           {getFieldDecorator(`purchaseAmountHkd${record.uuid}`, {
    //             initialValue: record?.purchaseAmountHkd,
    //             rules: [
    //               {
    //                 required: true,
    //                 message: intl.get('hzero.common.validation.notNull', {
    //                   name: intl.get(`${promptCode}.view.title.purchaseamount`).d('采购申请金额（HKD）'),
    //                 }),
    //               },
    //             ],
    //           })(
    ...
    //               precision={2}
    //               onChange={(e) => { record.purchaseAmountHkd = e }}
    //               disabled={isEdit}
    //             />
    ...
    //   // },
    //   // {
    //   //   title: intl.get(`${promptCode}.view.title.otherfeeamount`).d('其他费用金额（原币）'),
    ...
    const columns = [
      ...
      //     return (
      //       <Form.Item>
      //         {record.$form.getFieldDecorator(`purchaseAmountHkd`, {
      //           initialValue: record?.purchaseAmountHkd,
      //           rules: [
      //             {
      //               required: true,
      ...
      // },
      {
        ...
        render: (_, record) => {
          return (
            isEdit ? tooltipRender(record.lastPoNumber) :
            <Form.Item>
    ...
    return (
      <>
        <EditTable
          rowKey={ROW_KEY}
          pagination={budgetInfoListPagination}
          columns={columns}
