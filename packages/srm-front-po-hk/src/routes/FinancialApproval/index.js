...
import queryString from 'querystring';
import PriceInfoList from '../CusPriceSummaryDetail/PriceInfoList';
import PriceSummaryTalbe from '../CusPriceSummaryDetail/PriceSummaryTalbe';
import ProcurementForm from '../CusPriceSummaryDetail/ProcurementForm'; 
...
@connect(({ purchaseApplicationModel, purchaseApplicationCusModel, loading }) => ({
  purchaseApplicationModel,
  purchaseApplicationCusModel,
  fetchLoading: loading.effects['purchaseApplicationModel/handlePurchaseLineInfo'],
}))
export default class purchaseInquiryQuery extends React.Component {
  ...
  handlePriceSumList() {
    ...
    dispatch({
      type: `purchaseApplicationCusModel/getPriceTolList`,
      payload: {
  ...
  handleSearch = (page = {}) => {
    ...
    dispatch({
      type: 'purchaseApplicationModel/handlePurchaseLineInfo',
      payload: {
        page,
        prThirdHeadId,
      },
      ...
      const list = res.purchaseLineInfo.content.map((item) => {
        ...
        if (item.budgetType === 'CAPEX') {
          if (budgetNumberCapex) {
            budgetNumberCapex += `${item?.budgetNumber && item?.budgetNumber !== '/' ? `/${item?.budgetNumber}` : ''}`;
          } else {
            budgetNumberCapex += `${item?.budgetNumber}`;
          }
          if (entryNameCapex) {
            entryNameCapex += `${item?.entryName && item?.entryName !== '/' ? `/${item?.entryName}` : ''}`;
          } else {
            entryNameCapex += `${item?.entryName}`;

          }
          budgetItemNumberCapex += budgetItemNumberCapex === '' ? `${item?.budgetItemNumber}` : `${item?.budgetItemNumber && item?.budgetItemNumber !== '/' ? `/${item?.budgetItemNumber}` : ''}`;
          costCenterCapex += costCenterCapex === '' ? `${item?.costCenter}` : `${item?.costCenter && item?.costCenter !== '/' ? `/${item?.costCenter}` : ''}`;
          businessActivitiesCapex += businessActivitiesCapex === '' ? `${item?.businessActivities}` : `${item?.businessActivities && item?.businessActivities !== '/' ? `/${item?.businessActivities}` : ''}`;
          ...
          if (budgetNumberOpex) {
            budgetNumberOpex += `${item?.budgetNumber && item?.budgetNumber !== '/' ? `/${item?.budgetNumber}` : ''}`;
          } else {
            budgetNumberOpex += `${item?.budgetNumber}`;
          }
          if (entryNameOpex) {
            entryNameOpex += `${item?.entryName && item?.entryName !== '/' ? `/${item?.entryName}` : ''}`;
          } else {
            entryNameOpex += `${item?.entryName}`;
          }
          budgetItemNumberOpex += budgetItemNumberOpex === '' ? `${item?.budgetItemNumber}` : `${item?.budgetItemNumber && item?.budgetItemNumber !== '/' ? `/${item?.budgetItemNumber}` : ''}`;
          costCenterOpex += costCenterOpex === '' ? `${item?.costCenter}` : `${item?.costCenter && item?.costCenter !== '/' ? `/${item?.costCenter}` : ''}`;
          businessActivitiesOpex += businessActivitiesOpex === '' ? `${item?.businessActivities}` : `${item?.businessActivities && item?.businessActivities !== '/' ? `/${item?.businessActivities}` : ''}`;
          ...
          if (budgetNumberMix) {
            budgetNumberMix += `${item?.budgetNumber && item?.budgetNumber !== '/' ? `/${item?.budgetNumber}` : ''}`;
          } else {
            budgetNumberMix += `${item?.budgetNumber}`;
          }
          if (entryNameMix) {
            entryNameMix += `${item?.entryName && item?.entryName !== '/' ? `/${item?.entryName}` : ''}`;
          } else {
            entryNameMix += `${item?.entryName}`;
          }
          budgetItemNumberMix += budgetItemNumberMix === '' ? `${item?.budgetItemNumber}` : `${item?.budgetItemNumber && item?.budgetItemNumber !== '/' ? `/${item?.budgetItemNumber}` : ''}`;
          costCenterMix += costCenterMix === '' ? `${item?.costCenter}` : `${item?.costCenter && item?.costCenter !== '/' ? `/${item?.costCenter}` : ''}`;
          businessActivitiesMix += businessActivitiesMix === '' ? `${item?.businessActivities}` : `${item?.businessActivities && item?.businessActivities !== '/' ? `/${item?.businessActivities}` : ''}`;
  ...
  save(callback) {
    ...
    const { prName, prNumber, totalAmount } = purchaseApplicationModel;
    const { prThirdHeadId } = this.state;
  ...
  render() {
    const { idpValueMap = {}, fetchLoading = false, purchaseApplicationModel , form} = this.props;
    const { activeKey, searchTabActiveKey, projectInfo } = this.state;
    const { purchaseApproveForm, uuid } = purchaseApplicationModel;
    const baseInfoProps = {
      purchaseApplicationModel,
      idpValueMap,
      onRef: (ref) => {
    ...
    const messageTitle = (() => {
      ...
      if(['SCM04', 'SCM05', 'SCM06'].includes(activityCode)) {
