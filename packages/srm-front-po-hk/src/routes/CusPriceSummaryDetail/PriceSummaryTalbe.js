...
export default class PriceSummaryTalbe extends PureComponent {
  ...
  onSearchBtnClick = (record) => {
    ...
    dispatch({
      type: 'purchaseApplicationCusModel/getMatList',
      payload: {
  ...
  handleSaveMaterial = () => {
    ...
    if(selectedRowKeys.length > 0) {
      if (params.length > 0) {
        dispatch({
          type: 'purchaseApplicationCusModel/priceSummaryMatsave',
          payload: [
            {
  ...
  handlePriceSummarySave = (cb = (e) => e) => {
    ...
    dispatch({
      ...
      payload: [
        {
          refHeadId: lineRecord.refHeadId,
          refSupId: lineRecord.refSupId,
          rounds: lineRecord.rounds,
          supName: lineRecord.supName,
  ...
  render() {
    ...
    const rowSelection = {
      ...
      getCheckboxProps: (record) => ({
        disabled:
          isEdit || record.isQuote === 'N', // 选择框的是否可选
      }),
    };
