...
import PurchaseEditMatComponent from '@/components/PurchaseEditMatComponent';
...
@Form.create()
@connect(({ purchaseApplicationCusModel, loading }) => ({
  purchaseApplicationCusModel,
  fetchLoading:
    loading.effects['purchaseApplicationCusModel/demanderEditQuery'] ||
    loading.effects['purchaseApplicationCusModel/demanderCreateQuery'] ||
    loading.effects['purchaseApplicationCusModel/getInquireDetail'],
  submitLoading:
...
export default class purchaseInquiryQuery extends React.Component {
  ...
  constructor(props) {
    ...
    console.log('refHeadId', refHeadId)
    this.state = {
      activeKey: [
        'form',
        'applicationTable',
        ...
        if (['SUBMIT', 'DRAFT_HANDLE'].includes(e.data.submitType)) {
                  ...
                  formData: {
                    ...
                    affairTitle: intl
                      .get('HKPC.commom.view.title.bpmPAApproval', {
                        ...
                        amount: numberRender(params.amount, 2),
                      })
                      .d(
          ...
          this.save((params) => {
            if (params) {
              ...
              // // 首次提交的时候触发金额检验
              // dispatch({
              //   type: 'purchaseApplicationCusModel/getPriceValidate',
              //   payload: {
  ...
  query() {
    ...
    dispatch({
      type: `purchaseApplicationCusModel/demanderEditQuery`,
      payload: {
  ...
  handleInquireDetail = () => {
    ...
    const { prThirdHeadId } = this.state;
    dispatch({
      type: `purchaseApplicationCusModel/getInquireDetail`,
      payload: {
        ...
        if (budgetInfoData.length > 0) {
          this.budgetInfoForm.validateFieldsAndScroll((err2) => {
            ...
            if (budgetInfoErr.length === 0) {
              dispatch({
                type: 'purchaseApplicationCusModel/priceSummarySave',
                payload: priceSumList
              })
              // 保存需要的基本信息参数
              console.log('this.baseForm.current', this.baseForm.current.getFieldsValue());
              dispatch({
                type: `purchaseApplicationCusModel/demanderSave`,
                payload: {
  ...
  handleOk() {
    const { dispatch, purchaseApplicationCusModel } = this.props;
    const { refPrFirstId, budgetInfoList } = purchaseApplicationCusModel;
      ...
      if (!err) {
        if (setRowsItem) {
          CusModal.confirm({
            ...
            onOk: () => {
              dispatch({
                type: 'purchaseApplicationCusModel/projectEditName',
                payload: {
  ...
  changeCheck(record, index) {
    const { dispatch, purchaseApplicationCusModel } = this.props;
    const { reviewPriceData = [] } = purchaseApplicationCusModel;
  ...
  handleComparePricebutton() {
    const { dispatch, purchaseApplicationCusModel } = this.props;
    const { prThirdHeadId } = this.state;
    const { priceSummaryList = [], priceBasicInfo = {} } = purchaseApplicationCusModel;
    const { projectNumber, currency } = priceBasicInfo;

    dispatch({
      type: `purchaseApplicationCusModel/getReviewPrice`,
      payload: {
        id: prThirdHeadId,
        projectNumber,
        currency,
  ...
  handleSave() {
    ...
    const newSelectData = Array.isArray(selectData) ? (selectData || []) : [selectData]
    const params = newSelectData.map((item) => {
      return {
        refHeadId: prThirdHeadId,
    ...
    dispatch({
      type: `purchaseApplicationCusModel/getPriceSummaryList`,
      payload: params || [],
    }).then((res) => {
      if (res) {
        dispatch({
  ...
  handleOkMat = () => {
    const { dispatch, purchaseApplicationCusModel } = this.props;
    const { editMatDataSource } = purchaseApplicationCusModel;
    ...
    if(params.length > 0) {
      dispatch({
        type: 'purchaseApplicationCusModel/saveEditMat',
        payload: params
      }).then((res) => {
        if(res) {
          this.setState({
  ...
  render() {
    ...
    const { reviewPriceData = [], priceBasicInfo = {}, projectType } = purchaseApplicationCusModel;
    ...
    const inquireInfoProps = {
      purchaseApplicationCusModel,
      isEdit: state === 'DONE' || state === 'REVOKE' || (activityCode && activityCode !== 'Start'),
      onRef: (ref) => {
    ...
    return (
      <>
                  ...
                  children: (
                    ...
                    {searchTabActiveKey === 'purchase' ? (
                      <div style={{ height: '100vh' }}>
                        <iframe
                          style={{ width: '100%', height: '100%' }}
                          src={`/pub/ssrc-hk/purchaseApplicationErp/edit?id=${purchaseApplicationCusModel?.refPrFirstId}`}
                          width="100%"
                          height="100% !important"
                          frameBorder="0"
                        />
        ...
        >
          <PurchaseEditMatComponent {...editMatComponentProps} />
        </CusModal>
        {/* <CusApprovalButtons
          children={
            <CusButton
