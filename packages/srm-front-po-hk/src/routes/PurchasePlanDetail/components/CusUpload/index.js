...
export default class Upload extends PureComponent {
  ...
  onUploadSuccess(file, fileList) {
    ...
    fileList.map(item => {
      ...
      item.createdUser = getCurrentUser().loginName
      item.fileSize = item.size
      item.filePath = item.url
      item.refType = item.name
      item.fileName = item.name  //后端修改字段名称
      item.attachmentUuid = this.state.attachmentUUID  //后端要这个uuid
    })
    this.setState({
      fileList,
    }, () => {
  ...
  async removeFile(file) {
    ...
    if (file.url) {
      ...
      if (getResponse(res)) {
        ...
        if (this.upload) {
          this.upload.setFileList(fileList.filter((list_1) => list_1.url !== file.url));
        }
        if (removeCallback) {
          removeCallback();
        }
  ...
  @Bind()
  changeButton(flag) {
    this.setState({ closeLoading: flag });
  }
  ...
  render() {
    const {
      ...
      hasTemplate,
      multiple = true,
      btnText = intl.get('hzero.common.upload.text').d('上传'),
      description,
      onChange,
      tenantId,
      btnProps = {},
    ...
    const uploadLinkButton = (
      <div style={{ float: 'left' }} className="cus-upload-tag">
        {isEmpty(btnProps) ? (
          <CusButton disabled={prStatus == 'Approved' ? true : false} mini type="primary" onClick={this.openUploadModal}>
            {btnText}
          </CusButton>
        ) : (
          <CusButton disabled={prStatus == 'Approved' ? true : false} onClick={this.openUploadModal} {...btnProps}>
            {btnText}
          </CusButton>
        )}
    ...
    const modalContent = (
      <React.Fragment>
        ...
        <div style={{ display: loading ? 'none' : '', overflow: 'hidden' }}>
          {descriptionBlock}
          <CusSpin spinning={modalLoading}>
            <UploadButton
              changeButton = {this.changeButton}
              delSelectedRows={this.props.delSelectedRows}
