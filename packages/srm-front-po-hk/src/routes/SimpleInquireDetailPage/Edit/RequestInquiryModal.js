...
export default class RfqResponse extends React.Component {
  ...
  */
  ...
  openfimeEmailModal(record) {
    ...
    // 这里还要调一个获取预览信息的接口
    dispatch({
      type: `purchaseApplicationModel/supplierPreview`,
      payload: {
        proId: refHeadId,
  ...
  */
  ...
  sendEmail(record) {
    ...
    const { dispatch, stagePreview = (e) => e, purchaseApplicationModel } = this.props;
    const { refHeadId } = purchaseApplicationModel;
    // 发送
    dispatch({
  ...
  render() {
    const { querySupplierLoading = false, purchaseApplicationModel, isSendAllFlag } = this.props;
    const { fileEmailModel } = this.state;
    const { supplierList = [], stageList } = purchaseApplicationModel;
    const columns = [
      {
