 ...
 */
  fetchEnum = () => {
    ...
    dispatch({
      type: 'purchaseApplicationCusModel/fetchDetailEnum',
    });
  }

  handleAddMat = () => {
    const { purchaseApplicationCusModel, dispatch } = this.props;
    const { editMatDataSource, priceBasicInfo } = purchaseApplicationCusModel;
    const { currency, projectType } = priceBasicInfo;
  ...
  render() {
    ...
    const { priceBasicInfo = {}, editMatDataSource = [], detailEnumMap = {} } = purchaseApplicationCusModel;
    const { projectNumber, projectType } = priceBasicInfo;
