...
class LovModal extends React.Component {
  ...
  @Bind()
  querySearchData(pagination = {}, cb = (e) => e) {
    // const filter = this.props.form.getFieldsValue();
    const filter = this.form?.current?.getFieldsValue() || {};
    const {
      queryUrl = '',
    ...
    return tableFields.map((item) => {
      ...
      switch (dataType) {
        ...
        case 'RIGHT':
          ...
          break;
        case 'HYPERLINK':
          updatedItem.render = (val, record) => (
            <a href='#' onClick={() => this.handleHyperlinkClick(item.dataIndex, record)}>{val}</a>
          );
          break;
        default:
          break;
  ...
  @Bind()
  handleHyperlinkClick(dataIndex, record){
    const { hyperlinks = {} } =  this.props;
    if(isFunction(hyperlinks[dataIndex])){
      const url = hyperlinks[dataIndex](record);
      window.open(url);
    }
  }
