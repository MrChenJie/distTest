...
export default class TLEditor extends React.Component {
  constructor(props) {
    super(props);
    const tlsName = this.props.id ? `${this.props.id}#_tls` : '_tls'; // 全表编辑使用一个表单时，字段需要拼接id
    this.state = {
  ...
  componentDidMount() {
    const { tlsName } = this.state;
    // 初始化时清空表单多语言字段
    this.props.form?.setFieldsValue({ [tlsName]: null });
  }
  ...
  save(data) {
    ...
    const { list, language, tlsName } = this.state;
    // 设置多语言后，构建编辑的数据结构
    ...
    if (form && isUndefined(form.getFieldValue(tlsName))) {
      // 往外层form配置_tls表单域
      // form.registerField(tlsName);
      // 设置_tls值
      form.setFieldsValue({ [tlsName]: { [fieldName]: data } });
    } else {
      const oldTls = form.getFieldValue(tlsName);
      const newTls = { ...oldTls, [fieldName]: data };
      form.setFieldsValue({ [tlsName]: newTls });
    }
    // 更新input显示值
  ...
  openModal() {
    ...
    const { list, text, language, resLangList = [], tlsName } = this.state;
    if (Array.isArray(resLangList) && resLangList.length === 0) {
      ...
      queryTL({ fieldName, _token }).then((res) => {
        if (getResponse(res)) {
          ...
          // 往外层form配置_tls表单域
          // form.setFields({ tlsName: {} });
          if (form && isUndefined(form.getFieldValue(tlsName))) {
            // 设置_tls值
            form.setFieldsValue({ [tlsName]: { [fieldName]: tls } });
          } else {
            const oldTls = form.getFieldValue(tlsName);
  ...
  @Bind()
  onChange(e) {
    const { resLangList, list, language, tlsName } = this.state;
    const { form, field: fieldName = '' } = this.props;
    this.setState({ text: e.target.value });
    const tls = form.getFieldValue(tlsName);
    if (!tls) {
      form.setFieldsValue({ [tlsName]: { [fieldName]: { [language]: e.target.value } } });
    ...
    const suffix = (
      <>
        ...
        <img
          ...
          style={{ cursor: 'pointer', color: '#666', width: '16px' }}
        />
      </>
    );
    return (
