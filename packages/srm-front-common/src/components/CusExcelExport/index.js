...
import { isTenantRoleLevel, listenDownloadError, getAccessToken } from 'utils/utils';
import request from '@/utils/request';
import { downloadFile, initiateAsyncExport } from 'hzero-front/lib/services/api';
...
export default class ExcelExport extends React.PureComponent {
  ...
  @Bind()
  showSyncExportModal() {
    const { onExportCheck } = this.props;
    if (isFunction(onExportCheck)) {
      onExportCheck().then(res => {
        if (res) {
          this.showModal(true);
        }
      })
  ...
  render() {
    ...
    const exportEntryElement =
      exportAsync && isTenantRoleLevel() && isEmpty(defaultConfig) ? (
        ...
        <CusButton {...buttonProps} onClick={this.showSyncExportModal} loading={exportPending || exportCheckLoading}>
          {buttonText}
        </CusButton>
      );
    const rowSelection = {
