...
class MyComponentDict extends Component {
  ...
  componentDidMount() {
    ...
    dispatch({
      type: 'registerBidModel/getToken',
    }).then((res) => {
      if (res?.access_token) {
        dispatch({
        ...
        dispatch({
          type: 'registerBidModel/getBidNotice',
          payload: {
  ...
  handleBusinessLicenseUpload = (file) => {
    ...
    dispatch({
      type: 'registerBidModel/uploadFile',
      payload: {
        formData,
  ...
  @Bind
  handleReset() {
    this.currencyFormRef.current.resetFields('currencyCode');
    this.currencyFormRef.current.resetFields('currencyName');
  }
  // 查询所有币种
  @Bind
  getCurrency(page = {}) {
    ...
    const { access_token } = registerBidModel;
    let fieldsValue = {};
    if (this.currencyFormRef.current) {
      fieldsValue = this.currencyFormRef.current.getFieldsValue();
      fieldsValue = {
    ...
    dispatch({
      type: 'registerBidModel/getAllCurrencyCode',
      payload: {
        infos: { ...fieldsValue, page: { ...page, pageSize: 100, current: 1 } },
  ...
  getExchangeRate(code) {
    ...
    dispatch({
      type: 'registerBidModel/getScaleByCurrency',
      payload: {
        headers: { Authorization: access_token },
        infos: {
          currency: code || '',
  ...
  handleOkClose() {
    ...
    const { dispatch } = this.props;
    dispatch({
      type: 'registerBidModel/register',
      payload: {
        ...resultText,
      },
    }).then((res) => {
              ...
              <Form ref={this.currencyFormRef}>
                ...
                <Row gutter={16}>
                  ...
                  >
                    ...
                    <Button onClick={this.getCurrency} type="primary" style={{ marginLeft: '8px' }}>
                      {this.translate(`${prompt}.view.modal.query`)}
                    </Button>
                  </Col>
                </Row>
              </Form>
              <Table {...listProps} style={{ maxHeight: '500px', overflow: 'auto' }} />
