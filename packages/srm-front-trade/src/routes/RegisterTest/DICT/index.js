...
import { Form, Input, Select, Button, Upload, Col, Row, Radio, InputNumber, Spin,message } from 'antd';
...
class MyComponentDict extends Component {
  ...
  componentDidMount() {
    ...
    dispatch({
      type: 'registerDICTModel/getToken',
    }).then((res) => {
      if (res?.access_token) {
        dispatch({
  ...
  onFinish = () => {
    ...
    this.formRef.current
      ...
      .then((values) => {
        dispatch({
          type: 'registerDICTModel/register',
          payload: {
            access_token,
            ...values,
  ...
  getUrl = () => {
    ...
    const { currentLanguage } = this.state;
    dispatch({
      type: 'registerDICTModel/queryLovData',
      payload: {
        access_token,
        lovCode: 'DICT.CONFIG',
  ...
  getPartnerModeOptions = () => {
    ...
    if (access_token) {
      dispatch({
        type: 'registerDICTModel/queryLovViewInfo',
        payload: {
          access_token,
  ...
  handleCertificateUpload = (file) => {
    ...
    dispatch({
      type: 'registerDICTModel/uploadFile',
      payload: {
        formData,
  ...
  render() {
    ...
    const { registerDICTModel } = this.props;
    const { access_token } = registerDICTModel;
    const downLoadCodeconductUrl = `${HZERO_FILE}/v1/0/files/decrypt-download-ext?bucketName=template&access_token=${access_token}
    &url=${encodeURIComponent(codeconductUrl)}`;
    const downLoadCompliancecommitmentUrl = `${HZERO_FILE}/v1/0/files/decrypt-download-ext?bucketName=template&access_token=${access_token}
