...
import { isObject, isString, isUndefined, remove, uniqWith, isEmpty } from 'lodash';
...
export default class UploadButton extends React.Component {
  ...
  componentDidUpdate() {
    this.removeLabelTitle();
  }

  @Bind()
  removeLabelTitle() {
    let name = document.getElementsByClassName('ant-upload-list-item-name');
    if (!isEmpty(name)) {
      name[0].removeAttribute('title');
    }
  }
  ...
  beforeUpload(file) {
    ...
    // const maxFileSize = configureParams?.maxFileSize || 30 * 1024 * 1024;
