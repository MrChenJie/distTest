...
```
> 注意:  
> 1. 执行完 lerna bootstrap 会在 node_modules 下面生成一个 srm-front-core-2-common 的软链接，链接指向 packages/srm-front-core-2-common  
> 2. 所以在其他子模块里面可以直接引用 srm-front-core-2-common 模块的文件， 可以把 srm-front-core-2-common 看成一个公共依赖  
> 3. 执行完 lerna run transpile 之后会生成 packages/srm-front-core-2-common/lib 文件夹, 模块之间的相互依赖都是通过 lib 目录暴露出去的，如果 lib 文件的代码更新了 ,改动的代码才会生效  
...
```bash
cd packages/srm-front-core-2-demo1
yarn run start
```
